...
class CutinappCheckinTokenPrivacyTest extends TestCase
{
    ...
    public function test_operational_responses_do_not_expose_qr_token_or_cross_event_pass_data(): void
    {
        ...
        $operatorProfile = Profile::create([
            'name' => 'Cutinapp Privacy Checkin Operator',
        ...
        $claimedPass = $this->withHeaders($this->headersFor($participant))
            ->postJson('/api/cutinapp/passes/claim/' . $secondTicket['id'])
            ->assertCreated()
            ->json('pass');

        $this->assertArrayHasKey('token', $claimedPass, 'O participante precisa receber o token do próprio ingresso para gerar o QR Code.');
        ...
        DB::table('application_user')->updateOrInsert(
            ...
            [
                'role' => 'staff',
                'status' => 'active',
                'metadata' => json_encode(['event_ids' => [$firstEvent['id']]], JSON_THROW_ON_ERROR),
                'joined_at' => now(),
                'created_at' => now(),
                'updated_at' => now(),
            ]
        ...
        $participants = $this->withHeaders($this->headersFor($owner))
            ->getJson('/api/cutinapp/events/' . $secondEvent['id'] . '/participants')
            ->assertOk()
            ->json('passes');
        ...
        $managedPass = $this->withHeaders($this->headersFor($owner))
            ->getJson('/api/cutinapp/passes/' . $claimedPass['id'])
            ->assertOk()
            ->json('pass');

        $this->assertArrayNotHasKey('token', $managedPass);

        $checkinPass = $this->withHeaders($this->headersFor($owner))
            ->postJson('/api/cutinapp/checkin', [
        ...
        $ownPass = $this->withHeaders($this->headersFor($participant))
            ->getJson('/api/cutinapp/passes/' . $claimedPass['id'])
            ->assertOk()
            ->json('pass');

        $this->assertSame($token, $ownPass['token'] ?? null, 'O participante deve continuar conseguindo abrir o QR Code do próprio ingresso.');
    }
    ...
    private function createPublishedEventWithCourtesy(User $owner, int $productionId, string $title): array
    {
        $event = $this->withHeaders($this->headersFor($owner))
            ->postJson('/api/cutinapp/events', [
                ...
                'title' => $title,
                'description' => 'Evento usado para validar privacidade dos tokens de ingresso.',
                'address' => 'Rua Privacidade, 10',
        ...
        $ticket = $this->withHeaders($this->headersFor($owner))
