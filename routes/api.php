...
Route::post('/invite', [AuthController::class, 'invite'])->middleware(['api', 'auth:api', 'throttle:10,1'])->name('invite');
...
Route::prefix('user')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::put('/{user}', [UserController::class, 'update'])->whereNumber('user')->name('user.update.put');
...
Route::prefix('production')->middleware('api')->group(function () {
    ...
    Route::get('/cnpj/get-company-info', [ProductionController::class, 'getCompanyInfo'])->middleware('throttle:30,1')->name('production.getCompanyInfo');
    Route::get('/{slug}', [ProductionController::class, 'view'])->where('slug', '[A-Za-z0-9\-]+')->name('production.view');
});
Route::prefix('production')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::match(['post', 'put'], '/{id}', [ProductionController::class, 'update'])->whereNumber('id')->name('production.update');
...
Route::prefix('event')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::match(['post', 'put'], '/{id}', [EventController::class, 'update'])->whereNumber('id')->name('event.update');
    Route::delete('/{id}', [EventController::class, 'delete'])->whereNumber('id')->name('event.delete');
});
Route::prefix('event')->middleware('api')->group(function () {
    Route::get('/{slug}', [EventController::class, 'view'])->where('slug', '[A-Za-z0-9\-]+')->name('event.view');
});
...
Route::prefix('ticket')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::match(['post', 'put'], '/{id}', [TicketController::class, 'update'])->whereNumber('id')->name('ticket.update');
...
Route::prefix('news')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::match(['post', 'put'], '/{id}', [NewsController::class, 'update'])->whereNumber('id')->name('news.update');
...
Route::prefix('establishment')->middleware('api')->group(function () {
    ...
    Route::get('/list-others/{slug}', [EstablishmentController::class, 'listOthers'])->where('slug', '[A-Za-z0-9\-]+')->name('establishment.listOthers');
...
Route::prefix('order')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::post('/direct', [OrderController::class, 'storeDirect'])->middleware('order.access:direct')->name('order.storeDirect');
    Route::get('/list-by-client/{app_id}', [OrderController::class, 'listByClient'])->whereNumber('app_id')->name('order.listByClient');
    Route::get('/list-by-entity-slug/{slug}', [OrderController::class, 'listByEntitySlug'])->middleware('order.access:entity')->name('order.listByEntitySlug');
    Route::get('/listbyemployer', [OrderController::class, 'listByEmployer'])->name('order.listByEmployer');
    Route::get('/view/{id}', [OrderController::class, 'view'])->whereNumber('id')->middleware('order.access:order')->name('order.view');
    Route::get('/listmy/{app_id}', [OrderController::class, 'listMy'])->whereNumber('app_id')->name('order.listMy');
    Route::put('/{id}/update-status', [OrderController::class, 'updateOrderStatus'])->whereNumber('id')->middleware('order.access:manage')->name('order.updateOrderStatus');
    Route::put('/{id}', [OrderController::class, 'update'])->whereNumber('id')->middleware('order.access:manage')->name('order.update');
    Route::get('/{id}', [OrderController::class, 'show'])->whereNumber('id')->middleware('order.access:order')->name('order.show');
});
...
Route::prefix('item')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::match(['post', 'put'], '/{id}', [ItemController::class, 'update'])->whereNumber('id')->name('item.update');
...
Route::prefix('file')->middleware(['api', 'auth:api'])->group(function () {
    ...
    Route::match(['post', 'put'], '/{id}', [FileController::class, 'update'])->whereNumber('id')->name('file.update');
