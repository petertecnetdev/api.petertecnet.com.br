...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('cutinapp_producer_payment_accounts', function (Blueprint $table) {
            ...
            $table->string('provider', 40)->default('efi');
            $table->string('status', 30)->default('pending');
            $table->string('pix_key', 255)->nullable();
            $table->string('provider_recipient_id', 255)->nullable();
        ...
        Schema::create('cutinapp_payouts', function (Blueprint $table) {
            $table->id();
            $table->uuid('public_id')->unique();
            $table->foreignId('production_id')->constrained('productions')->restrictOnDelete();
            $table->foreignId('payment_account_id')->constrained('cutinapp_producer_payment_accounts')->restrictOnDelete();
            $table->string('provider', 40)->default('efi');
            $table->string('status', 30)->default('pending');
            $table->decimal('amount', 12, 2);
            $table->string('provider_payout_id', 255)->nullable()->unique();
            $table->json('provider_payload')->nullable();
    ...
    {
        Schema::dropIfExists('cutinapp_payouts');
