...
return new class extends Migration {
 public function up():void{
  if(in_array(DB::getDriverName(),['mysql','mariadb'],true)&&Schema::hasTable('events')){
   foreach(['facebook_url','twitter_url','instagram_url','youtube_url','website','registration_link'] as $column){
    if(Schema::hasColumn('events',$column)) DB::statement("ALTER TABLE `events` MODIFY `{$column}` TEXT NULL");
   }
  }
  ...
  Schema::table('productions',function(Blueprint $t){if(!Schema::hasColumn('productions','city_id'))$t->unsignedBigInteger('city_id')->nullable()->index('prod_city_id_idx');if(!Schema::hasColumn('productions','address_number'))$t->string('address_number',30)->nullable();if(!Schema::hasColumn('productions','neighborhood'))$t->string('neighborhood',160)->nullable();if(!Schema::hasColumn('productions','address_complement'))$t->string('address_complement',255)->nullable();if(!Schema::hasColumn('productions','address_reference'))$t->string('address_reference',255)->nullable();if(!Schema::hasColumn('productions','formatted_address'))$t->text('formatted_address')->nullable();if(!Schema::hasColumn('productions','latitude'))$t->decimal('latitude',10,7)->nullable();if(!Schema::hasColumn('productions','longitude'))$t->decimal('longitude',10,7)->nullable();if(!Schema::hasColumn('productions','place_id'))$t->string('place_id',255)->nullable();if(!Schema::hasColumn('productions','google_maps_url'))$t->text('google_maps_url')->nullable();if(!Schema::hasColumn('productions','location_public'))$t->boolean('location_public')->default(false);});
  Schema::table('events',function(Blueprint $t){if(!Schema::hasColumn('events','city_id'))$t->unsignedBigInteger('city_id')->nullable()->index('event_city_id_idx');if(!Schema::hasColumn('events','event_format'))$t->string('event_format',20)->default('in_person')->index('event_format_idx');if(!Schema::hasColumn('events','address_number'))$t->string('address_number',30)->nullable();if(!Schema::hasColumn('events','neighborhood'))$t->string('neighborhood',160)->nullable();if(!Schema::hasColumn('events','address_complement'))$t->string('address_complement',255)->nullable();if(!Schema::hasColumn('events','address_reference'))$t->string('address_reference',255)->nullable();if(!Schema::hasColumn('events','formatted_address'))$t->text('formatted_address')->nullable();if(!Schema::hasColumn('events','place_id'))$t->text('place_id')->nullable();if(!Schema::hasColumn('events','online_platform'))$t->string('online_platform',120)->nullable();if(!Schema::hasColumn('events','online_url'))$t->text('online_url')->nullable();if(!Schema::hasColumn('events','online_instructions'))$t->text('online_instructions')->nullable();});
 }
