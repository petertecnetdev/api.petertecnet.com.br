...
class CatalogImageAnalysisService
{
    ...
    public function analyze(array $images, string $locale = 'pt-BR', string $currency = 'BRL'): array
    {
        ...
        if ($apiKey === '') {
            throw new RuntimeException('A importação inteligente de cardápio ainda não está configurada no servidor.');
        }
        ...
        if (! $response->successful()) {
            report(new RuntimeException(sprintf(
                'Catalog image analysis failed with HTTP %d: %s',
                $response->status(),
                Str::limit((string) $response->body(), 1000)
            )));
            throw new RuntimeException('Não foi possível analisar o cardápio agora. Tente novamente.');
        }

        $payload = $response->json();
        $decoded = json_decode($this->extractOutputText(is_array($payload) ? $payload : []), true);
...
Idioma de referência: {$locale}. Moeda: {$currency}.
Preserve o nome comercial e corrija apenas erros óbvios de leitura. Separe categoria e subcategoria quando estiverem claras.
Preço deve ser número decimal sem símbolo; use null quando ilegível. Não invente preço, descrição, marca ou SKU.
Quando houver tamanhos/variações com preços diferentes, gere um item por variação e inclua a variação no nome.
Use type "product" para comidas, bebidas, mercadorias e itens vendáveis; use "service" somente para serviços explícitos.
confidence deve ficar entre 0 e 1. source_text deve trazer um trecho curto que sustente a leitura quando útil.
Ignore cabeçalhos, telefones, endereços, redes sociais, formas de pagamento e textos institucionais.
Não duplique o mesmo item apenas porque ele aparece em mais de uma imagem.
Use warnings para imagem cortada, preço ilegível ou seção ambígua.
            ...
            'properties' => [
                'items' => [
                    ...
                    'items' => [
                        'type' => 'object',
                        'additionalProperties' => false,
                        'required' => ['name', 'description', 'price', 'category', 'subcategory', 'brand', 'sku', 'type', 'confidence', 'source_text'],
                        'properties' => [
                            'name' => ['type' => 'string'],
