...
final class EventDiscoveryController extends Controller
{
    ...
    {
        $data=$request->validate(['q'=>'nullable|string|max:120','city'=>'nullable|string|max:120','uf'=>'nullable|string|size:2','category'=>'nullable|string|max:120','artist_id'=>'nullable|integer|min:1','production_id'=>'nullable|integer|min:1','period'=>'nullable|in:today,tomorrow,weekend,friday,saturday,sunday,next7,next30,month,custom','date'=>'nullable|date_format:Y-m-d','from'=>'nullable|date_format:Y-m-d','to'=>'nullable|date_format:Y-m-d|after_or_equal:from','free'=>'nullable|boolean','available'=>'nullable|boolean','sort'=>'nullable|in:soonest,nearest,newest,popular','lat'=>'nullable|numeric|between:-90,90|required_with:lng','lng'=>'nullable|numeric|between:-180,180|required_with:lat','radius_km'=>'nullable|integer|min:1|max:500','per_page'=>'nullable|integer|min:1|max:50']);
        ...
        if(!empty($data['city']))$query->whereRaw('LOWER(events.city) = LOWER(?)',[trim($data['city'])]); if(!empty($data['uf']))$query->where('events.uf',strtoupper($data['uf'])); if(!empty($data['category']))$query->where('events.category',$data['category']); if(!empty($data['production_id']))$query->where('events.production_id',$data['production_id']); if(!empty($data['artist_id']))$query->whereHas('artists',fn($q)=>$q->where('artists.id',$data['artist_id']));
        ...
        if(!empty($data['q'])){$term='%'.trim($data['q']).'%';$query->where(fn($q)=>$q->where('events.title','like',$term)->orWhere('events.city','like',$term)->orWhere('events.venue','like',$term)->orWhere('events.category','like',$term)->orWhereHas('production',fn($p)=>$p->where('name','like',$term))->orWhereHas('artists',fn($a)=>$a->where('stage_name','like',$term)));}
