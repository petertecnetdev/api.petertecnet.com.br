...
final class EventManagementController extends Controller
{
    ...
    public function mine(Request $request)
    {
        ...
        $appId=$this->context->id();$now=Carbon::now(config('app.timezone','America/Sao_Paulo'));$query=Event::query()->where('app_id',$appId)->whereHas('production',fn($q)=>$q->where('app_id',$appId)->where('user_id',$user->id))->with(['production:id,app_id,name,slug,user_id,app_slug','artists:id,app_id,slug,stage_name'])->withCount(['tickets'=>fn($q)=>$q->where('app_id',$appId),'tickets as available_tickets_count'=>fn($q)=>$q->where('app_id',$appId)->where('quantity','>',0)->where(fn($ticket)=>$ticket->whereNull('limit_date')->orWhere('limit_date','>',$now))]);
    ...
    public function show(Request $request,int $id){$event=$this->ownedEvent($id,$request->user());$appId=$this->context->id();$now=Carbon::now(config('app.timezone','America/Sao_Paulo'));$event->load(['production:id,app_id,name,slug,user_id,app_slug','artists:id,app_id,slug,stage_name']);$event->loadCount(['tickets'=>fn($q)=>$q->where('app_id',$appId),'tickets as available_tickets_count'=>fn($q)=>$q->where('app_id',$appId)->where('quantity','>',0)->where(fn($ticket)=>$ticket->whereNull('limit_date')->orWhere('limit_date','>',$now))]);return response()->json(['event'=>$event]);}
