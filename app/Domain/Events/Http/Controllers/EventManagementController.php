...
final class EventManagementController extends Controller
{
    ...
    private function attachPaymentReadiness($events): void
    {
        ...
        foreach($events as$event){
            ...
            $readiness=$byProduction[$productionId]??[
                ...
                'methods'=>[],
                'message'=>'A plataforma de pagamentos ainda não está disponível para esta organização.',
                'payout_ready'=>false,
    ...
    private function validateDates(array $data, ?Event $event): void
    {
        ...
        $startChanged = $event === null;
        $endChanged = $event === null;

        if ($event !== null) {
            if (array_key_exists('start_date', $data)) {
                $originalStartValue = $event->getRawOriginal('start_date') ?? $event->start_date;
                $originalStart = $originalStartValue ? Carbon::parse((string) $originalStartValue, $timezone) : null;
                $startChanged = ! $originalStart || $originalStart->format('Y-m-d H:i:s') !== $start->format('Y-m-d H:i:s');
            }

            if (array_key_exists('end_date', $data)) {
                $originalEndValue = $event->getRawOriginal('end_date') ?? $event->end_date;
                $originalEnd = $originalEndValue ? Carbon::parse((string) $originalEndValue, $timezone) : null;
                $endChanged = ! $originalEnd || $originalEnd->format('Y-m-d H:i:s') !== $end->format('Y-m-d H:i:s');
            }
        }

        $errors = [];

        if ($event === null && $start->lte($now)) {
            $errors['start_date'][] = 'O horário de início do evento precisa estar no futuro.';
        } elseif ($event !== null && $startChanged && $start->lt($now->copy()->subMinute())) {
            $errors['start_date'][] = 'O início do evento não pode ficar no passado.';
        }

        if ($event === null || $startChanged || $endChanged) {
            if (! $end->gt($start)) {
    ...
    private function storeImage($file):string{$directory='images/apps/'.$this->context->slug().'/events';$path=$directory.'/'.Str::uuid().'.webp';$image=Image::make($file)->orientate()->fit(1024,1536)->encode('webp',88);Storage::disk('public')->put($path,(string)$image);return$path;}
