...
final class EventManagementController extends Controller
{
    ...
    public function mine(Request $request)
    {
        ...
        $data = $request->validate([
            'q'=>'nullable|string|max:120','city'=>'nullable|string|max:120','status'=>'nullable|in:draft,published,cancelled,postponed,rescheduled,upcoming,past','production_id'=>'nullable|integer|min:1','from'=>'nullable|date_format:Y-m-d','to'=>'nullable|date_format:Y-m-d|after_or_equal:from','per_page'=>'nullable|integer|min:1|max:100'
        ]);
        ...
        $query = Event::query()
            ...
            ->withCount(['tickets'=>fn($q)=>$q->where('app_id',$appId), 'commerceOrders as orders_count'=>fn($q)=>$q->where('app_id',$appId)]);
        ...
        match ($data['status'] ?? null) {
            ...
            'cancelled' => $query->where(fn($q)=>$q->where('is_cancelled',true)->orWhere('lifecycle_status','cancelled')),
            'postponed' => $query->where('lifecycle_status', 'postponed'),
            'rescheduled' => $query->where('lifecycle_status', 'rescheduled'),
    ...
    public function show(Request $request, int $id)
    {
        ...
        $event->loadCount(['tickets'=>fn($q)=>$q->where('app_id',$appId),'commerceOrders as orders_count'=>fn($q)=>$q->where('app_id',$appId)]);
        return response()->json(['event'=>$event,'lifecycle_impact'=>$this->lifecycle->impact($event)]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $data['lifecycle_status'] = EventLifecycleService::SCHEDULED;
        unset($data['image']);
    ...
    public function update(Request $request, int $id)
    {
        ...
        if (isset($data['production_id'])) $this->ownedProduction((int)$data['production_id'], $request->user());

        $dateChanged = false;
        if (array_key_exists('start_date', $data)) $dateChanged = ! $event->start_date || Carbon::parse($data['start_date'])->ne($event->start_date);
        if (array_key_exists('end_date', $data)) $dateChanged = $dateChanged || ! $event->end_date || Carbon::parse($data['end_date'])->ne($event->end_date);
        if ($dateChanged && $this->lifecycle->hasCommercialHistory($event)) {
            abort(409, 'Este evento possui histórico comercial. Use o fluxo de adiar/reagendar para alterar a data e proteger os participantes.');
        }

        $this->validateDates($data, $event);
        if (! empty($data['title']) && $data['title'] !== $event->title) $data['slug'] = $this->uniqueSlug($data['title'], $event->id);
        unset($data['image'],$data['app_id'],$data['app_slug'],$data['is_published'],$data['is_cancelled'],$data['lifecycle_status'],$data['sales_paused_at'],$data['cancelled_at'],$data['postponed_at'],$data['rescheduled_at'],$data['refund_deadline_at']);
        $event->update($data);
    ...
    public function publish(Request $request, int $id)
    {
        ...
        abort_if($event->is_cancelled || $event->lifecycle_status === EventLifecycleService::CANCELLED, 422, 'Um evento cancelado não pode ser publicado.');
        abort_if($event->lifecycle_status === EventLifecycleService::POSTPONED, 422, 'Defina a nova data pelo fluxo de reagendamento antes de republicar este evento.');
    ...
    public function destroy(Request $request,int $id)
    {
        ...
        abort_unless($impact['can_delete'],409,'Este evento possui vendas, pedidos ou ingressos emitidos e não pode ser excluído. Use cancelar ou adiar/reagendar para preservar o histórico.');
        $event->delete();
    ...
    private function normalizeInput(Request $request): void
    {
        ...
        foreach(['start_date','end_date'] as $field){
            ...
            catch(Throwable){$errors[$field][]=$field==='start_date'?'Informe uma data de início válida.':'Informe uma data de término válida.';}
        }
    ...
    private function validateDates(array $data,?Event $event): void
    {
        ...
        if($event===null&&$start->lte($now))$errors['start_date'][]='O horário de início do evento precisa estar no futuro.';
        elseif($event!==null&&array_key_exists('start_date',$data)&&$start->lt($now->copy()->subMinute()))$errors['start_date'][]='O início do evento não pode ficar no passado.';
    ...
    private function ownedProduction(int $id,User $user): Production
    {
        ...
        abort_unless($user->hasProfile('Administrador')||(int)$production->user_id===(int)$user->id,403,'Você não pode gerenciar esta organização.');
        return $production;
    }
    ...
    {
        $directory='images/apps/'.$this->context->slug().'/events';$path=$directory.'/'.Str::uuid().'.webp';$image=Image::make($file)->orientate()->resize(1920,1080,function($c){$c->aspectRatio();$c->upsize();})->encode('webp',86);Storage::disk('public')->put($path,(string)$image);return$path;
    }
