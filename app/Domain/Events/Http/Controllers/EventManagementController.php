...
final class EventManagementController extends Controller
{
    ...
    public function show(Request $request,int $id){$event=$this->ownedEvent($id,$request->user());$appId=$this->context->id();$event->load(['establishment:id,app_id,name,slug,user_id,type','artists:id,app_id,slug,stage_name']);$event->loadCount(['tickets'=>fn($q)=>$q->where('app_id',$appId)]);return response()->json(['event'=>$event]);}
