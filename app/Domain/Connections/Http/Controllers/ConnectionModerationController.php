...
final class ConnectionModerationController extends Controller
{
    ...
    public function reports(Request $request)
    {
        ...
        $reports = DB::table('connection_reports as reports')
    ...
    public function act(Request $request, int $reportId)
    {
        ...
        $report = DB::table('connection_reports')
            ->where('app_id', $this->context->id())
            ->where('id', $reportId)
            ->first();
        abort_unless($report, 404, 'Denúncia não encontrada.');

        $expiresAt = $data['action'] === 'suspend'
            ? now()->addHours((int) ($data['duration_hours'] ?? 72))
            : null;
