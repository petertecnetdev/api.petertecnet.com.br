...
class DiscoveryGrowthController extends Controller
{
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        return response()->json(['success' => true, 'data' => [
            'providers' => $this->searchPerformance->status(),
            'search_performance' => $search,
            'opportunities' => $this->learning->opportunities($days),
            'attribution' => $this->learning->attribution($days),
            'experiments' => $this->learning->experimentStats(),
            'accessibility' => $this->learning->accessibilitySummary($days),
            'public_health' => $this->learning->healthSummary(),
    ...
    public function syncSearchPerformance(Request $request): JsonResponse
    {
        ...
        $data = $request->validate([
            'provider' => ['nullable', 'string', 'in:google,bing'],
            'days' => ['nullable', 'integer', 'min:2', 'max:90'],
        ]);
    ...
    public function importSearchPerformance(Request $request): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'rows' => ['required', 'array', 'max:25000'],
            'rows.*.date' => ['required', 'date'],
            'rows.*.query' => ['nullable', 'string', 'max:500'],
            'rows.*.page' => ['nullable', 'string', 'max:1000'],
            'rows.*.device' => ['nullable', 'string', 'max:32'],
            'rows.*.country' => ['nullable', 'string', 'max:16'],
            'rows.*.clicks' => ['nullable', 'numeric', 'min:0'],
            'rows.*.impressions' => ['nullable', 'numeric', 'min:0'],
            'rows.*.ctr' => ['nullable', 'numeric', 'min:0'],
            'rows.*.position' => ['nullable', 'numeric', 'min:0'],
        ]);
    ...
    {
        $this->authorizeActor($request);
        return response()->json(['success' => true, 'data' => ['documents' => $this->learning->rebuildIndex()]]);
    }
    ...
    public function storeExperiment(Request $request): JsonResponse
    {
        ...
        $id = DB::table('discovery_experiments')->insertGetId([
            ...
            'key' => $data['key'], 'name' => $data['name'], 'surface' => $data['surface'], 'status' => $data['status'] ?? 'draft',
    ...
    public function updateExperiment(Request $request, int $experiment): JsonResponse
    {
        ...
        $payload = collect($data)->mapWithKeys(function ($value, $key) {
            if (in_array($key, ['variants', 'metadata'], true)) return [$key => $value === null ? null : json_encode($value)];
            return [$key => $value];
        })->all();
    ...
    private function qualityScores(): array
    {
        ...
        ContentEntry::query()->limit(1000)->get()->each(function ($entry) use ($rows) {
            $checks = [
                'title' => mb_strlen(trim((string) $entry->title)) >= 20,
                'description' => mb_strlen(trim((string) ($entry->seo_description ?: $entry->excerpt))) >= 70,
                'content' => mb_strlen(trim(strip_tags((string) $entry->content))) >= 450,
                'image' => (bool) ($entry->cover_image || $entry->og_image),
                'taxonomy' => (bool) ($entry->category || $entry->cluster || count($entry->tags ?: [])),
            ];
        ...
        Establishment::query()->where('is_cancelled', false)->limit(1000)->get()->each(function ($row) use ($rows) {
            ...
            $checks = ['name' => mb_strlen(trim((string) $name)) >= 3, 'description' => mb_strlen(trim((string) $row->description)) >= 80, 'location' => (bool) ($row->city && $row->uf), 'category' => (bool) ($row->category || $row->type), 'contact' => (bool) ($row->phone || $row->email || $row->site)];
        ...
        return [
            'average' => $rows->count() ? round((float) $rows->avg('score'), 1) : null,
            'ready' => $rows->where('score', '>=', 80)->count(),
            'needs_attention' => $rows->where('score', '<', 60)->count(),
            'rows' => $rows->sortBy('score')->take(100)->values()->all(),
        ];
    }
