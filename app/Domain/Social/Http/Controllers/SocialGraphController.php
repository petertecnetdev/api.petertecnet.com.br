...
final class SocialGraphController extends Controller
{
    ...
    {
        $artist=Artist::query()->where('app_id',$this->context->id())->where('slug',$slug)->where('is_published',true)->where('is_active',true)->firstOrFail();$artist->setAttribute('followers_count',$this->followersCount('artist',$artist->id));$artist->setAttribute('is_following',$this->isFollowing($request,'artist',$artist->id));
    ...
    private function canManageArtist(Artist $artist,User $user):bool{return$user->hasProfile('Administrador')||(int)$artist->user_id===(int)$user->id||(int)$artist->created_by_user_id===(int)$user->id||DB::table('artist_managers')->where(['app_id'=>$this->context->id(),'artist_id'=>$artist->id,'user_id'=>$user->id])->exists();}
