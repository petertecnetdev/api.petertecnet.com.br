...
final class SocialGraphController extends Controller
{
    ...
    {
        $data=$request->validate(['target_type'=>'required|in:user,artist,production','target_id'=>'required|integer|min:1']);
        abort_if($data['target_type']==='user' && (int)$data['target_id']===(int)$request->user()->id,422,'Você não pode seguir o próprio perfil.');
        $this->assertTarget($data['target_type'],$data['target_id']);
        DB::table('follows')->updateOrInsert(['app_id'=>$this->context->id(),'user_id'=>$request->user()->id,'target_type'=>$data['target_type'],'target_id'=>$data['target_id']],['updated_at'=>now(),'created_at'=>now()]);
    ...
    {
        $data=$request->validate(['target_type'=>'required|in:user,artist,production','target_id'=>'required|integer|min:1']);
        DB::table('follows')->where(['app_id'=>$this->context->id(),'user_id'=>$request->user()->id,'target_type'=>$data['target_type'],'target_id'=>$data['target_id']])->delete();
    ...
    public function engagement(Request $request,int $eventId)
    {
        ...
        $event=Event::query()->where('app_id',$appId)->where('is_published',true)->where('is_cancelled',false)->where(fn($query)=>$query->where('is_private',false)->orWhereNull('is_private'))->with('production:id,user_id,name,slug')->findOrFail($eventId);
        $data=$request->validate(['is_favorite'=>'sometimes|boolean','is_interested'=>'sometimes|boolean']);
        if(($data['is_interested']??false) && ! $event->allowedActions()['mark_interested']) abort(422,'Este evento já foi encerrado e não aceita novas marcações de interesse.');
        $key=['app_id'=>$appId,'user_id'=>$request->user()->id,'event_id'=>$event->id];$previous=DB::table('event_engagements')->where($key)->first();$wasInterested=(bool)($previous->is_interested??false);
        DB::table('event_engagements')->updateOrInsert($key,array_merge($data,['updated_at'=>now(),'created_at'=>now()]));$engagement=DB::table('event_engagements')->where($key)->first();$isInterested=(bool)($engagement->is_interested??false);$producerUserId=(int)($event->production?->user_id??0);$actor=$request->user();
        if(!$wasInterested&&$isInterested&&$producerUserId>0&&$producerUserId!==(int)$actor->id){$actorName=trim(implode(' ',array_filter([$actor->first_name,$actor->last_name])))?:($actor->user_name?:'Alguém');try{app(AppNotificationService::class)->sendToUser($appId,$producerUserId,['type'=>'event_interest','title'=>'Novo interesse no seu evento','message'=>Str::limit("{$actorName} demonstrou interesse em {$event->title}.",500),'reference_type'=>'event','reference_id'=>$event->id,'reference_url'=>'/event/'.$event->slug,'data'=>['actor_user_id'=>(int)$actor->id,'actor_name'=>$actorName,'production_id'=>$event->production_id,'event_slug'=>$event->slug]]);}catch(\Throwable $e){report($e);}}
    ...
    {
        if($type==='user'){
            abort_unless(User::query()->whereKey($id)->exists(),404,'Usuário não encontrado.');
            return;
        }
        if($type==='artist'){
            abort_unless(Artist::query()->where('app_id',$this->context->id())->whereKey($id)->where('is_published',true)->exists(),404,'Artista não encontrado.');
            return;
        }
        abort_unless(Production::query()->where('app_id',$this->context->id())->whereKey($id)->where('is_published',true)->where('is_cancelled',false)->exists(),404,'Organização não encontrada.');
    }
