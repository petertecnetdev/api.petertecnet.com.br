...
final class ParticipantSocialController extends Controller
{
        ...
        $query = User::query()
            ...
            ->where(function ($visibility) use ($followingIds) {
                ...
                if ($followingIds->isNotEmpty()) {
                    $visibility->orWhereIn('users.id', $followingIds);
                }
            })
            ->distinct();
    ...
    public function show(Request $request, int $participantId)
    {
        ...
        $participantInterests = $settings['show_interests']
            ? $this->interestsFor((int) $participant->id, $appId)
            : [];
        ...
        $viewerEventIds = DB::table('event_engagements')
            ->where(['app_id' => $appId, 'user_id' => $viewer->id, 'is_interested' => true])
            ->pluck('event_id');

        $participantEventIds = collect();
        if ($settings['show_event_interests']) {
            $participantEventIds = DB::table('event_engagements')
                ->where(['app_id' => $appId, 'user_id' => $participant->id, 'is_interested' => true])
                ->pluck('event_id');
        }
    ...
    public function activity(Request $request)
    {
        ...
        if ($followedParticipantIds->isEmpty()) {
            return response()->json(['activity' => new LengthAwarePaginator([], 0, $perPage)]);
        }
    ...
    {
        return response()->json([
            'settings' => $this->socialSettings((int) $request->user()->id, $this->context->id()),
        ]);
    }
    ...
    public function updateSettings(Request $request)
    {
        ...
        DB::table('user_social_preferences')->updateOrInsert(
        ...
        return response()->json([
            ...
            'settings' => $this->socialSettings($userId, $appId),
        ]);
    }
    ...
    private function decorateParticipants(Collection $participants, int $viewerId, int $appId): Collection
    {
        ...
        $followersBack = DB::table('follows')
            ->where(['app_id' => $appId, 'target_type' => 'user', 'target_id' => $viewerId])
            ->whereIn('user_id', $ids)
            ->pluck('user_id')->map(fn ($id) => (int) $id)->flip();
        $followerCounts = DB::table('follows')
            ->select('target_id', DB::raw('COUNT(*) as total'))
            ->where(['app_id' => $appId, 'target_type' => 'user'])
            ->whereIn('target_id', $ids)
            ->groupBy('target_id')->pluck('total', 'target_id');
        $sharedEventCounts = collect();
        if ($viewerEventIds->isNotEmpty()) {
            $sharedEventCounts = DB::table('event_engagements')
                ->select('user_id', DB::raw('COUNT(*) as total'))
                ->where('app_id', $appId)->where('is_interested', true)
                ->whereIn('user_id', $ids)->whereIn('event_id', $viewerEventIds)
                ->groupBy('user_id')->pluck('total', 'user_id');
        }

        $mutualFollowingCounts = collect();
        if ($viewerFollowingIds->isNotEmpty()) {
            $mutualFollowingCounts = DB::table('follows')
                ->select('user_id', DB::raw('COUNT(*) as total'))
                ->where('app_id', $appId)->where('target_type', 'user')
                ->whereIn('user_id', $ids)->whereIn('target_id', $viewerFollowingIds)
                ->groupBy('user_id')->pluck('total', 'user_id');
        }
    ...
    private function mutualFollowingCount(int $leftUserId, int $rightUserId, int $appId): int
    {
        ...
        return DB::table('follows')
            ->where(['app_id' => $appId, 'user_id' => $rightUserId, 'target_type' => 'user'])
            ->whereIn('target_id', $leftTargets)
            ->count();
    }
    ...
    {
        return DB::table('follows')
            ->where(['app_id' => $appId, 'user_id' => $userId, 'target_type' => 'user'])
    ...
    {
        $row = DB::table('user_social_preferences')->where(['app_id' => $appId, 'user_id' => $userId])->first();
        return $this->normalizeSocialSettings($row);
    }
