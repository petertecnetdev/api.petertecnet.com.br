...
final class SocialGraphService
{
    ...
    public function follow(Request $request){$data=$request->validate(['target_type'=>'required|in:artist,establishment','target_id'=>'required|integer|min:1']);$this->assertTarget($data['target_type'],$data['target_id']);DB::table('follows')->updateOrInsert(['app_id'=>$this->context->id(),'user_id'=>$request->user()->id,'target_type'=>$data['target_type'],'target_id'=>$data['target_id']],['updated_at'=>now(),'created_at'=>now()]);return response()->json(['message'=>'Agora você está seguindo este perfil.','following'=>true]);}
    public function unfollow(Request $request){$data=$request->validate(['target_type'=>'required|in:artist,establishment','target_id'=>'required|integer|min:1']);DB::table('follows')->where(['app_id'=>$this->context->id(),'user_id'=>$request->user()->id,'target_type'=>$data['target_type'],'target_id'=>$data['target_id']])->delete();return response()->json(['message'=>'Você deixou de seguir este perfil.','following'=>false]);}
    ...
    {
        if($type==='artist'){abort_unless(Artist::query()->where('app_id',$this->context->id())->whereKey($id)->where('is_published',true)->exists(),404,'Artista não encontrado.');return;}
        abort_unless(Establishment::query()->where('app_id',$this->context->id())->where('type','production')->whereKey($id)->where('is_published',true)->where('is_cancelled',false)->exists(),404,'Estabelecimento não encontrado.');
    }
    private function ownedEvent(int $id,User $user):Event{$event=Event::where('app_id',$this->context->id())->with('establishment')->findOrFail($id);abort_unless($event->establishment&&$event->establishment->type==='production'&&($user->hasProfile('Administrador')||(int)$event->establishment->user_id===(int)$user->id),403,'Você não pode gerenciar este evento.');return$event;}
