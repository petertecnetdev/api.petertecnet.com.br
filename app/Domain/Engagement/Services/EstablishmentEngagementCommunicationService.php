...
{
    public function __construct(private readonly EstablishmentEngagementReportService $reportService)
    {
    }
    ...
    public function send(int $applicationId, int $establishmentId, ?int $actorId, ?string $ip, ?string $userAgent): array
    {
        $application = $this->application($applicationId);
        $establishment = $this->establishment($applicationId, $establishmentId);
        [$recipientEmail, $recipientName] = $this->recipient($establishment);
        ...
        $trackedCtas = collect($report['ctas'])->map(function (array $cta) use ($communicationId, $establishment, $application) {
            return array_merge($cta, [
                'url' => $this->trackedClickUrl($communicationId, (string) $cta['url'], (string) $cta['key'], (int) $establishment->id, (int) $application->id),
            ]);
        })->all();

        $trackingPixelUrl = URL::temporarySignedRoute('communications.track.open', now()->addDays(30), [
            'communicationId' => $communicationId,
        ...
        return ['communication_id'=>$communicationId,'recipient'=>$recipientEmail,'subject'=>$report['subject']];
    }

    private function establishment(int $applicationId, int $establishmentId): Establishment
    {
        return Establishment::query()->where('app_id', $applicationId)->where('category', 'production')
            ->with('user:id,first_name,last_name,email')->findOrFail($establishmentId);
    }
    ...
    private function recipient(Establishment $establishment): array
    {
        ...
        if ($email === '' || ! filter_var($email, FILTER_VALIDATE_EMAIL)) {
            throw ValidationException::withMessages(['recipient' => ['A produção não possui um e-mail válido para receber o resumo.']]);
        }
        $userName = trim(implode(' ', array_filter([$establishment->user?->first_name, $establishment->user?->last_name])));
    ...
    {
        return URL::temporarySignedRoute('communications.track.click', now()->addDays(30), [
            'communicationId'=>$communicationId,
