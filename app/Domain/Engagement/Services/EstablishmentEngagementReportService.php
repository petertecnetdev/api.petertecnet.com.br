...
final class EstablishmentEngagementReportService
{
    public function generate(Establishment $establishment, Application $application): array
    {
        ...
        if ($nextEvent?->slug) {
            array_splice($ctas, 1, 0, [[
                ...
                'url' => $baseUrl.'/event/'.$nextEvent->slug,
                'primary' => false,
            ]]);
        }
        ...
        $metrics = [
            ...
            'events_future' => $futureEvents->count(),
            'events_published' => $publishedEvents->count(),
            'events_draft' => $draftEvents->count(),
    ...
    private function insights(array $metrics, ?Event $nextEvent, Collection $draftEvents): array
    {
        ...
        if ($metrics['events_future'] === 0) {
            $insights[] = ['type' => 'opportunity', 'title' => 'Mantenha sua agenda ativa', 'message' => 'Sua produção ainda não tem um próximo evento publicado. Cadastrar a próxima data mantém sua página ativa e dá ao público um motivo para voltar.'];
        }

        if ($draftEvents->isNotEmpty()) {
            $insights[] = ['type' => 'action', 'title' => 'Você tem evento aguardando publicação', 'message' => 'Há '.$draftEvents->count().' evento(s) em rascunho. Revise as informações e publique quando estiver pronto para começar a divulgação e as vendas.'];
        }

        if ($metrics['event_views'] >= 100 && $metrics['conversion_rate'] !== null && $metrics['conversion_rate'] < 5) {
            $insights[] = ['type' => 'opportunity', 'title' => 'Seu evento chama atenção, mas pode converter mais', 'message' => 'Há tráfego na página, porém a proporção de ingressos vendidos ainda está baixa. Reforce o flyer, a oferta e o link direto de compra nas divulgações.'];
        }

        if ($nextEvent?->start_date && $nextEvent->start_date->between(now(), now()->addDays(7))) {
            $insights[] = ['type' => 'action', 'title' => 'Seu próximo evento está perto', 'message' => 'Este é um bom momento para redistribuir o flyer, compartilhar o link do evento e reforçar a chamada para compra antecipada.'];
        }
        ...
        if ($insights === []) {
            $insights[] = ['type' => 'positive', 'title' => 'Continue movimentando sua produção', 'message' => 'Mantenha eventos, informações e agenda atualizados. Cada atualização deixa sua página mais útil para o público e facilita novas vendas.'];
        }
