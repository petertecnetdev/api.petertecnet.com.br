...
final class OrderHistoryController extends Controller
{
    ...
    public function receipt(Request $request,string $publicId){$order=$this->findOrder($publicId);$this->authorize($request,$order);return response()->json(['receipt'=>$this->receiptData($order)]);}
    public function receiptPdf(Request $request,string $publicId){$order=$this->findOrder($publicId);$this->authorize($request,$order);$receipt=$this->receiptData($order);$application=$this->context->application();return Pdf::loadView('pdf.order-receipt',compact('receipt','application'))->setPaper('a4')->download('recibo-'.substr($order->public_id,0,8).'.pdf');}
    ...
    private function authorize(Request $request,CommerceOrder $order):void{if((int)$order->user_id===(int)$request->user()->id)return;$this->ownedOrganization($request,(int)$order->production_id);}
