...
final class EventCommerceController extends Controller
{
    ...
    public function catalog(Request $request, string $slug)
    {
        ...
        $tickets = Ticket::query()
            ...
            ->each(function (Ticket $ticket) use ($salesClosed, $userId) {
                $this->appendUserTicketAvailability($ticket, $userId);

                if ($salesClosed || $ticket->getAttribute('remaining_per_user') === 0) {
                    $ticket->setAttribute('available', false);
                }
            });
    ...
    public function checkout(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'tickets.*.quantity' => 'required_with:tickets|integer|min:1|max:1000000',
        ...
        abort_if(
            $eventForReadiness->salesClosed(),
            422,
            $eventForReadiness->hasEnded()
        ...
        $order = DB::transaction(function () use ($data, $user, $platformRate, $expirationMinutes) {
            ...
            foreach (($data['tickets'] ?? []) as $requested) {
                ...
                $issued = $this->issuedTicketCount((int) $ticket->id);
                $reserved = $this->reservedTicketCount((int) $ticket->id);

                abort_if(
                    $issued + $reserved + $qty > (int) $ticket->quantity,
                    422,
                    "Não há quantidade suficiente no lote {$ticket->name}."
                );

                if ($ticket->max_per_user !== null) {
                    $purchasedByUser = $this->issuedTicketCount((int) $ticket->id, (int) $user->id);
                    $reservedByUser = $this->reservedTicketCount((int) $ticket->id, (int) $user->id);
                    $remainingForUser = max(0, (int) $ticket->max_per_user - $purchasedByUser - $reservedByUser);

                    abort_if(
                        $qty > $remainingForUser,
                        422,
                        $remainingForUser > 0
                            ? "Você ainda pode comprar {$remainingForUser} ingresso(s) do lote {$ticket->name}."
                            : "Você atingiu o limite de {$ticket->max_per_user} ingresso(s) por usuário no lote {$ticket->name}."
                    );
                }
            ...
            foreach (($data['items'] ?? []) as $requested) {
                ...
                $sold = DB::table('commerce_order_items as oi')
        ...
        return response()->json([
            'message' => $data['payment_method'] === 'pix'
                ? 'Pedido criado. Pague o PIX antes do vencimento.'
    ...
    public function show(Request $request, string $publicId)
    {
        $order = CommerceOrder::query()
            ...
            ->with(['items','event.production','payments'])
            ->firstOrFail();
    ...
    public function upsertEventItem(Request $request, int $eventId, ?int $itemId = null)
    {
        ...
        $item = $itemId
            ? EventItem::query()->where('app_id', $this->context->id())->where('event_id', $event->id)->findOrFail($itemId)
            : new EventItem(['app_id' => $this->context->id(), 'event_id' => $event->id]);
    ...
    public function financialSummary(Request $request, int $organizationId)
    {
        ...
        return response()->json([
            ...
            'organization_earned' => round($earned, 2),
    ...
    {
        if ($ticket->max_per_user === null) {
            $ticket->setAttribute('purchased_by_user', $userId > 0 ? $this->issuedTicketCount((int) $ticket->id, $userId) : 0);
            $ticket->setAttribute('reserved_by_user', $userId > 0 ? $this->reservedTicketCount((int) $ticket->id, $userId) : 0);
            $ticket->setAttribute('remaining_per_user', null);
            return;
        }

        if ($userId <= 0) {
            $ticket->setAttribute('purchased_by_user', 0);
            $ticket->setAttribute('reserved_by_user', 0);
            $ticket->setAttribute('remaining_per_user', (int) $ticket->max_per_user);
            return;
        }

        $purchased = $this->issuedTicketCount((int) $ticket->id, $userId);
        $reserved = $this->reservedTicketCount((int) $ticket->id, $userId);

        $ticket->setAttribute('purchased_by_user', $purchased);
        $ticket->setAttribute('reserved_by_user', $reserved);
        $ticket->setAttribute('remaining_per_user', max(0, (int) $ticket->max_per_user - $purchased - $reserved));
    }

    private function issuedTicketCount(int $ticketId, ?int $userId = null): int
    {
    ...
    private function reservedTicketCount(int $ticketId, ?int $userId = null): int
    {
        return (int) DB::table('inventory_reservations as ir')
            ->when($userId !== null, function ($query) use ($userId) {
                $query
                    ->join('commerce_orders as co', 'co.id', '=', 'ir.order_id')
                    ->where('co.app_id', $this->context->id())
                    ->where('co.user_id', $userId)
                    ->where('co.status', 'pending');
            })
            ->where('ir.app_id', $this->context->id())
            ->where('ir.ticket_id', $ticketId)
