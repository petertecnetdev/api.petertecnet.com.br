...
final class EventCommerceController extends Controller
{
    ...
    public function catalog(string $slug)
    {
        ...
        $salesPaused = $this->salesPaused($event);
        $paymentConfig = [
            'provider' => 'mercadopago',
            'connected' => $readiness['available'],
            'available' => $salesPaused ? false : $readiness['available'],
            'merchant_connected' => $readiness['merchant_connected'],
            'settlement_mode' => $readiness['settlement_mode'],
            'public_key' => $readiness['public_key'],
            'methods' => $salesPaused ? [] : $readiness['methods'],
            'message' => $salesPaused
                ? 'As vendas deste evento estão temporariamente pausadas por uma alteração de programação.'
                : $readiness['message'],
        ];

        return response()->json([
            'event' => $event->only([
                'id','title','slug','start_date','end_date','lifecycle_status','sales_paused_at','refund_deadline_at',
            ]),
            'tickets' => $tickets,
            'items' => $items,
    ...
    public function checkout(Request $request)
    {
        ...
        $eventForReadiness = Event::query()
            ...
            ->whereHas('production', fn ($q) => $q->where('app_id', $this->context->id()))
            ->firstOrFail();
        abort_if($this->salesPaused($eventForReadiness), 422, 'As vendas deste evento estão pausadas. Confira a atualização de programação antes de tentar novamente.');
        ...
        $order = DB::transaction(function () use ($data, $user, $platformRate, $expirationMinutes) {
            ...
            abort_if($this->salesPaused($event), 422, 'As vendas deste evento estão pausadas por uma alteração de programação.');
            ...
            foreach (($data['tickets'] ?? []) as $requested) {
                ...
                $reserved = DB::table('inventory_reservations')->where('app_id', $this->context->id())->where('ticket_id', $ticket->id)->whereNull('released_at')->where('expires_at','>',now())->sum('quantity');
                ...
                CommerceOrderItem::create(['app_id'=>$this->context->id(),'order_id'=>$order->id,'type'=>'ticket','ticket_id'=>$ticket->id,'name'=>$ticket->name,'unit_price'=>$ticket->price,'quantity'=>$qty,'subtotal'=>$line]);
                DB::table('inventory_reservations')->insert(['app_id'=>$this->context->id(),'order_id'=>$order->id,'type'=>'ticket','ticket_id'=>$ticket->id,'quantity'=>$qty,'expires_at'=>$expiresAt,'created_at'=>now(),'updated_at'=>now()]);
                $subtotal += $line;
            }

            foreach (($data['items'] ?? []) as $requested) {
                $item = EventItem::query()->where('app_id',$this->context->id())->where('id',$requested['id'])->where('event_id',$event->id)->where('is_active',true)->lockForUpdate()->firstOrFail();
                $reserved = DB::table('inventory_reservations')->where('app_id',$this->context->id())->where('event_item_id',$item->id)->whereNull('released_at')->where('expires_at','>',now())->sum('quantity');
                $sold = DB::table('commerce_order_items as oi')->join('commerce_orders as o','o.id','=','oi.order_id')->where('oi.app_id',$this->context->id())->where('o.app_id',$this->context->id())->where('oi.event_item_id',$item->id)->where('o.status','paid')->sum('oi.quantity');
                $qty = (int) $requested['quantity'];
                ...
                CommerceOrderItem::create(['app_id'=>$this->context->id(),'order_id'=>$order->id,'type'=>'item','event_item_id'=>$item->id,'name'=>$item->name,'unit_price'=>$item->price,'quantity'=>$qty,'subtotal'=>$line]);
                DB::table('inventory_reservations')->insert(['app_id'=>$this->context->id(),'order_id'=>$order->id,'type'=>'item','event_item_id'=>$item->id,'quantity'=>$qty,'expires_at'=>$expiresAt,'created_at'=>now(),'updated_at'=>now()]);
                $subtotal += $line;
            }
        ...
        try {
            ...
            if ($data['payment_method'] === 'card') $payer['identification'] = ['type'=>'CPF','number'=>$data['payer_identification_number']];
            ...
            $payload = [
                ...
                'payer'=>$payer,
                'metadata'=>['application_id'=>$this->context->id(),'order_id'=>$order->id,'order_public_id'=>$order->public_id,'organization_id'=>$order->production_id,'settlement_mode'=>$settlementMode],
            ];
            ...
            if (! empty($remote['_same_account'])) {
                $settlementMode = 'same_account';
                $order->update(['metadata'=>array_merge($order->metadata ?? [], ['settlement_mode'=>$settlementMode])]);
            }
    ...
    {
        return response()->json(['orders'=>CommerceOrder::query()->where('app_id',$this->context->id())->where('user_id',$request->user()->id)->with(['items','event','payments','refunds'])->latest()->paginate(20)]);
    }

    public function show(Request $request,string $publicId)
    {
        $order=CommerceOrder::query()->where('app_id',$this->context->id())->where('public_id',$publicId)->with(['items','event.production','payments','refunds'])->firstOrFail();
        abort_unless((int)$order->user_id===(int)$request->user()->id||$this->ownsOrganization($request,(int)$order->production_id),403);
    ...
    public function upsertEventItem(Request $request,int $eventId,?int $itemId=null)
    {
        ...
        $item=$itemId?EventItem::query()->where('app_id',$this->context->id())->where('event_id',$event->id)->findOrFail($itemId):new EventItem(['app_id'=>$this->context->id(),'event_id'=>$event->id]);
        $item->fill($data)->save();
    ...
    {
        $event=$this->ownedEvent($request,$eventId);
        $item=EventItem::query()->where('app_id',$this->context->id())->where('event_id',$event->id)->findOrFail($itemId);
    ...
    public function paymentAccount(Request $request,int $organizationId)
    {
        ...
        return response()->json(['account'=>['provider'=>$account->provider,'status'=>$account->status,'provider_recipient_id'=>$account->provider_recipient_id,'connected_at'=>$account->connected_at,'verified_at'=>$account->verified_at,'token_expires_at'=>$account->token_expires_at]]);
    }

    public function financialSummary(Request $request,int $organizationId)
    {
        $this->ownedOrganization($request,$organizationId);
        $gross=(float)DB::table('ledger_entries')->where('app_id',$this->context->id())->where('production_id',$organizationId)->where('type','gross_sale')->where('status','posted')->sum('amount');
        $fees=abs((float)DB::table('ledger_entries')->where('app_id',$this->context->id())->where('production_id',$organizationId)->where('type','platform_fee')->where('status','posted')->sum('amount'));
        $earned=(float)DB::table('ledger_entries')->where('app_id',$this->context->id())->where('production_id',$organizationId)->where('type','producer_credit')->where('status','posted')->sum('amount');
        $processorFees=(float)DB::table('commerce_payments as p')->join('commerce_orders as o','o.id','=','p.order_id')->where('p.app_id',$this->context->id())->where('o.app_id',$this->context->id())->where('o.production_id',$organizationId)->where('p.status','paid')->sum('p.provider_fee');
        $readiness=$this->accounts->readiness($organizationId);
        return response()->json(['gross_sales'=>round($gross,2),'platform_fees'=>round($fees,2),'processor_fees'=>round($processorFees,2),'organization_earned'=>round($earned,2),'settlement'=>$readiness['settlement_mode'],'sales_enabled'=>$readiness['available'],'sales_message'=>$readiness['message'],'provider'=>'mercadopago']);
    }

    private function salesPaused(Event $event): bool
    {
        return (bool) $event->sales_paused_at
            || in_array((string) $event->lifecycle_status, ['postponed','cancelled'], true);
    }

    private function cancelOrder(CommerceOrder $order):void
    {
        DB::transaction(function()use($order){CommerceOrder::query()->where('app_id',$this->context->id())->where('id',$order->id)->where('status','pending')->update(['status'=>'cancelled','cancelled_at'=>now()]);DB::table('inventory_reservations')->where('app_id',$this->context->id())->where('order_id',$order->id)->whereNull('released_at')->update(['released_at'=>now(),'updated_at'=>now()]);});
    }
    ...
    private function ownedEvent(Request $request,int $id):Event
    {
        ...
        $this->ownedOrganization($request,(int)$event->production_id);
        return$event;
    }
    ...
    private function ownedOrganization(Request $request,int $id):Production
    {
        ...
        abort_unless($this->ownsOrganization($request,$id),403);
        return$organization;
    }

    private function ownsOrganization(Request $request,int $id):bool
    {
        $organization=Production::query()->where('app_id',$this->context->id())->find($id);
        if(!$organization||!$request->user())return false;
        $admin=method_exists($request->user(),'hasProfile')&&$request->user()->hasProfile('Administrador');
        return$admin||(int)$organization->user_id===(int)$request->user()->id;
    }
}
