...
class CommerceFulfillmentService
{
    ...
    public function transition(Order $order, string $targetStatus, User $actor, array $context = []): Order
    {
        return DB::transaction(function () use ($order, $targetStatus, $actor, $context) {
            ...
            $current = $this->normalizeStatus($locked->status);

            abort_unless($locked->payment_status === 'paid', 422, 'O pagamento precisa estar confirmado antes do preparo.');
            abort_if($this->isTerminal($locked), 409, 'O recebimento deste pedido já foi concluído ou cancelado.');
            ...
            $allowed = match ($current) {
                '', 'pending', 'confirmed' => [self::PREPARING],
    ...
    public function verify(Order $order, ?string $token, ?string $code, ?User $actor = null, array $context = []): Order
    {
        if ($this->isTerminal($order)) {
            $this->record($order, 'credential_verified', $actor?->id, $this->presentedStatus($order), $this->presentedStatus($order), null, 'already_redeemed', $context);
            abort(409, 'Este comprovante já foi utilizado ou o pedido foi encerrado.');
        }

        abort_unless($order->payment_status === 'paid', 422, 'Pagamento ainda não confirmado.');
        abort_unless($this->isReady($order->status), 422, 'O pedido ainda não está pronto para retirada ou entrega.');

        $method = $this->credentialType($order, $token, $code);
        if (! $method) {
            $this->record($order, 'credential_verified', $actor?->id, self::READY, self::READY, null, 'invalid_credential', $context);
            abort(403, 'Comprovante ou código de retirada inválido.');
        }

        $this->record($order, 'credential_verified', $actor?->id, self::READY, self::READY, $method, 'success', $context);
    ...
    public function redeem(Order $order, ?string $token, ?string $code, User $actor, array $context = []): Order
    {
        return DB::transaction(function () use ($order, $token, $code, $actor, $context) {
            ...
            if ($this->isTerminal($locked)) {
                $status = $this->presentedStatus($locked);
            ...
            abort_unless($this->isReady($locked->status), 422, 'O pedido ainda não está pronto para retirada ou entrega.');

            $method = $this->credentialType($locked, $token, $code);
            if (! $method) {
                $this->record($locked, 'redeem_attempted', $actor->id, self::READY, self::READY, null, 'invalid_credential', $context);
                abort(403, 'Comprovante ou código de retirada inválido.');
            }
    ...
    {
        if ($order->payment_status !== 'paid' || ! $this->isReady($order->status)) {
            return null;
        }
    ...
    public function claimCode(Order $order): string
    {
        $digest = hash_hmac(
            'sha256',
            implode('|', ['commerce-fulfillment-code', $order->id, $order->app_id, $order->client_id, $order->order_number]),
            (string) config('app.key')
        );
    ...
    {
        return $this->normalizeStatus($status) === self::READY;
    }

    public function isTerminal(Order $order): bool
    {
        return in_array($this->normalizeStatus($order->status), ['completed', 'cancelled'], true);
    }

    public function presentedStatus(Order $order): string
    {
        $status = $this->normalizeStatus($order->status);

        if ($status === 'completed') {
            return $order->fulfillment === 'delivery' ? self::DELIVERED : self::FULFILLED;
        }
