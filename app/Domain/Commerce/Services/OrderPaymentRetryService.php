...
final class OrderPaymentRetryService
{
    ...
    {
        [$order, $establishment, $payment] = DB::transaction(function () use ($orderId, $user) {
            $order = Order::query()
                ->whereKey($orderId)
            ...
            return [$order, $establishment, $payment];
        }, 3);
        ...
        try {
            ...
            ], $this->context->slug().'-order-'.$order->id.'-payment-'.$payment->public_id);
