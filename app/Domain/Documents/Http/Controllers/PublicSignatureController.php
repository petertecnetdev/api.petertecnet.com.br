...
final class PublicSignatureController extends Controller
{
    ...
    public function sign(Request $request, string $token)
    {
        ...
        $document = $this->documents->sign($signatureRequest->document_id, $party->role, [
            'name' => $data['signer_name'], 'email' => $party->email, 'tax_id' => $data['signer_tax_id'] ?? $party->tax_id,
        ], null, $request);
    ...
    private function resolve(string $token, bool $mustBePending = false): object
    {
        ...
        if ($mustBePending) $query->where('status', 'pending');
