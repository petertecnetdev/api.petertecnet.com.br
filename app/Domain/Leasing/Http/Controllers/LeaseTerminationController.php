...
final class LeaseTerminationController extends Controller
{
    ...
    public function show(Request $request, int $leaseId)
    {
        ...
        $operation = DB::table('lease_operations')
            ...
            ->orderByDesc('id')
            ->first();
        $document = $this->documents->latestForContext($this->context->id(), 'lease', $leaseId, 'lease_termination');

        return response()->json([
            'termination' => $operation ? $this->decodeOperation($operation) : null,
            'document' => $document,
            'timeline' => $document ? $this->documents->timeline($document->id) : [],
        ]);
    }
    ...
    public function complete(Request $request, int $leaseId)
    {
        ...
        abort_if(in_array($lease->status, ['cancelled'], true), 422, 'Uma locação cancelada não pode receber distrato.');
        ...
        $mutualReleaseRequested = (bool) ($data['mutual_release'] ?? false);
        if ($mutualReleaseRequested) {
        ...
        if ($inspection) $checklist['exit_inspection'] = true;
        ...
        $settlement = [
            ...
            'financial' => array_merge($amounts, [
                'deposit_settlement' => $data['deposit_settlement'],
            ]),
            'utilities_notes' => trim((string) ($data['utilities_notes'] ?? '')) ?: null,
            'mutual_release' => $mutualReleaseRequested,
            'checklist' => $checklist,
        ...
        $content = $this->buildTerminationDocument($lease, $property, $settlement, $inspection, $agreement);
        ...
        DB::transaction(function () use ($lease, $operation, $payload, $settlement, $endedOn, $data) {
            DB::table('lease_operations')->where('id', $operation->id)->update([
                'status' => 'completed',
        ...
        return response()->json([
            'ok' => true,
            'termination' => $this->decodeOperation(DB::table('lease_operations')->find($operation->id)),
            'document' => $this->documents->payload($document->id),
            'pdf_path' => '/v1/apps/'.$this->context->slug().'/documents/'.$document->public_id.'/pdf',
        ]);
    }
    ...
    public function send(Request $request, int $leaseId)
    {
        ...
        try {
            foreach ($sent['signature_requests'] as $signatureRequest) {
                ...
                Mail::raw($this->invitationText($sent['document'], $link, $party->name), function ($mail) use ($party) {
                    $mail->to($party->email, $party->name)->subject('Distrato para assinatura eletrônica');
                });
                $recipients[] = $this->maskEmail($party->email);
            }
        } catch (Throwable $e) {
            report($e);
            $this->documents->reopenForRevision($document->id, (int) $request->user()->id, $request);
            return response()->json([
    ...
    private function buildTerminationDocument(object $lease, object $property, array $settlement, ?object $inspection, ?object $agreement): string
    {
    ...
    private function date(mixed $value): string
    {
        ...
        try { return CarbonImmutable::parse($value)->format('d/m/Y'); } catch (Throwable) { return (string) $value; }
    }

    private function dateTime(mixed $value): string
    {
        if (! $value) return 'não informada';
        try { return CarbonImmutable::parse($value)->format('d/m/Y H:i'); } catch (Throwable) { return (string) $value; }
    }
    ...
    private function decodeRow(object $row, array $jsonColumns): array
    {
        ...
        foreach ($jsonColumns as $column) {
            if (array_key_exists($column, $data)) $data[$column] = $this->jsonDecode($data[$column]);
        }
        return $data;
    }
