...
final class LeaseDocumentWorkflowController extends Controller
{
    ...
    public function generate(Request $request, int $leaseId)
    {
        ...
        DB::table('leases')->where('app_id', $this->context->id())->where('id', $leaseId)->update([
            'contract_version' => $document->current_version, 'status' => 'awaiting_signature', 'updated_at' => now(),
        ]);
    ...
    public function send(Request $request, int $leaseId)
    {
        ...
        try {
            ...
            report($e);
            $this->documents->reopenForRevision($document->id, (int) $request->user()->id, $request);
            return response()->json(['message' => 'O contrato não foi enviado. As solicitações de assinatura foram invalidadas com segurança; verifique a configuração de e-mail.'], 502);
        }

        return response()->json(['ok' => true, 'sent_to' => $this->maskEmail($lease->tenant_email), 'document' => $sent['document']]);
    }
    ...
    public function sign(Request $request, int $leaseId)
    {
        ...
        if ($data['party'] === 'landlord') {
            ...
            $isTenant = (int) $lease->tenant_user_id === $userId || (! $lease->tenant_user_id && $lease->tenant_email && strcasecmp($lease->tenant_email, (string) $request->user()->email) === 0);
            abort_unless($isTenant || $this->isAdmin($request), 403);
            if (! $lease->tenant_user_id && ! $this->isAdmin($request)) {
                DB::table('leases')->where('app_id', $this->context->id())->where('id', $leaseId)->update(['tenant_user_id' => $userId, 'updated_at' => now()]);
            }
        }

        $document = $this->documents->latestForContext($this->context->id(), 'lease', $leaseId, 'lease_agreement');
        abort_if(! $document, 422, 'O contrato ainda não foi gerado.');
        $document = $this->documents->sign($document->id, $data['party'], [
            'name' => $data['signer_name'], 'email' => $request->user()->email, 'tax_id' => $data['signer_tax_id'] ?? null,
        ], $userId, $request);
    ...
    public function storeAmendment(Request $request, int $leaseId)
    {
        $lease = $this->managedLease($request, $leaseId);
        $data = $request->validate([
            'title' => 'required|string|max:190', 'changes' => 'required|array|min:1|max:50',
            'changes.*.label' => 'required|string|max:190', 'changes.*.value' => 'required|string|max:5000',
        ]);
        ...
        $document = $this->documents->createOrRevise(
            $this->context->id(), 'lease_amendment', $leaseId.'-'.Str::uuid(), 'lease_amendment', $data['title'], $content,
            $payload, $this->parties($lease), (int) $request->user()->id, $agreement->id, $request,
        );
        return response()->json(['document' => $document, 'timeline' => $this->documents->timeline($document->id)], 201);
    }
    ...
    private function decodeObject(object $row, array $columns): array
    {
        ...
        foreach ($columns as $column) {
            if (array_key_exists($column, $data) && is_string($data[$column])) $data[$column] = json_decode($data[$column], true);
        }
        return $data;
    }
