...
final class OrganizationService
{
    ...
    {
        return Establishment::query()->where('app_id', $this->context->id())->where('type', 'production');
    }
    ...
    private function normalize(Request $request): void
    {
        ...
        if ($request->exists('instagram_url')) {
            ...
            if ($value === '') $merge['instagram_url'] = null;
            elseif (preg_match('#^https?://#i', $value)) $merge['instagram_url'] = $value;
            else {
                $handle = ltrim($value, '@/');
                $merge['instagram_url'] = str_contains($handle, 'instagram.com/') ? 'https://' . $handle : 'https://instagram.com/' . $handle;
            }
        }
