...
class AdminControlPlaneService
{
    ...
    public function capabilities()
    {
        return response()->json([
            ...
            'saved_views' => true,
            'trash_restore' => true,
            'moderation' => Schema::hasTable('connection_reports'),
        ]);
    }
    ...
    {
        if (! Schema::hasTable('connection_reports')) {
            return response()->json(['reports' => ['data' => []], 'summary' => ['open' => 0, 'resolved' => 0]]);
        }

        $query = DB::table('connection_reports as r')
        ...
        return response()->json([
            ...
            'summary' => [
                'open' => DB::table('connection_reports')->where('status', 'open')->count(),
                'resolved' => DB::table('connection_reports')->where('status', 'resolved')->count(),
                'dismissed' => DB::table('connection_reports')->where('status', 'dismissed')->count(),
                'actions' => Schema::hasTable('connection_moderation_actions') ? DB::table('connection_moderation_actions')->count() : 0,
            ],
        ]);
    }
    ...
    {
        abort_unless(Schema::hasTable('connection_reports'), 404);
        $validated = $request->validate([
        ...
        $row = DB::table('connection_reports')->where('id', $report)->first();
        abort_unless($row, 404);

        DB::transaction(function () use ($request, $report, $row, $validated) {
            DB::table('connection_reports')->where('id', $report)->update([
            ...
            if ($action !== 'none' && Schema::hasTable('connection_moderation_actions')) {
                DB::table('connection_moderation_actions')->insert([
                    'report_id' => $report,
                ...
                if (in_array($action, ['suspend', 'ban'], true) && Schema::hasTable('connection_profiles')) {
                    DB::table('connection_profiles')->where('user_id', $row->reported_user_id)->update(['discovery_enabled' => false, 'updated_at' => now()]);
                }
            }
        });

        $this->audit($request, 'moderation.update', 'connection_report', $report, $validated);
    ...
    public function trash(Request $request)
    {
        $resources = [
            ...
            'connection_messages' => ['label' => 'Mensagens removidas', 'name' => "CONCAT('Mensagem #', id)"],
        ];
        $rows = collect();
    ...
    {
        $allowed = ['establishments', 'productions', 'items', 'connection_messages'];
