...
final class SupportTicketService
{
    public function create(Request $request, array $data): array
    {
        ...
        $ticket = DB::transaction(function () use ($request, $data, $user, $application, $accessToken, $name, $email, $now) {
        ...
        return ['ticket' => $this->publicTicket($ticket), 'access_token' => $accessToken];
    }

    public function show(Request $request, string $publicId): array
    {
        $ticket = SupportTicket::query()->where('public_id', $publicId)->with([
    ...
    public function reply(Request $request, string $publicId, string $message): array
    {
        $ticket = SupportTicket::query()->where('public_id', $publicId)->firstOrFail();
        $user = $this->authorizeRequester($request, $ticket);
        DB::transaction(function () use ($ticket, $user, $message) {
            SupportMessage::create([
                'support_ticket_id' => $ticket->id, 'user_id' => $user?->id,
                'author_type' => 'requester', 'body' => trim($message), 'is_internal' => false,
            ]);
            $updates = ['last_message_at' => now()];
            if (in_array($ticket->status, ['waiting_customer', 'resolved', 'closed'], true)) {
                $updates['status'] = 'open'; $updates['resolved_at'] = null; $updates['closed_at'] = null;
            }
    ...
    public function mine(Request $request)
    {
        ...
        $tickets = SupportTicket::query()->where('user_id', $user->id)
            ->with(['application:id,name,slug'])->orderByDesc('last_message_at')->orderByDesc('id')
    ...
    {
        if (! empty($data['application_id'])) return Application::query()->find($data['application_id']);
        $slug = $this->sourceAppSlug($request, $data);
        return $slug === '' ? null : Application::query()->whereRaw('LOWER(slug) = ?', [$slug])->first();
    }
    ...
    {
        try { return $request->user('api'); } catch (\Throwable) { return null; }
    }

    private function authorizeRequester(Request $request, SupportTicket $ticket)
    {
        $user = $this->optionalUser($request);
        if ($user && (int) $ticket->user_id === (int) $user->id) return $user;
        $token = trim((string) ($request->header('X-Support-Token') ?: $request->query('token', '')));
        $valid = $token !== '' && $ticket->access_token_hash && hash_equals((string) $ticket->access_token_hash, hash('sha256', $token));
    ...
    {
        $messages = $ticket->relationLoaded('messages') ? $ticket->messages->where('is_internal', false)->values()->map(fn (SupportMessage $message) => [
        ...
        return array_filter([
            ...
            'resolved_at' => $ticket->resolved_at, 'created_at' => $ticket->created_at, 'updated_at' => $ticket->updated_at,
            'messages' => $messages,
