...
class AdminCopilotService
{
    ...
    public function preflight(array $actions, array $context = []): array
    {
        $resolved = [];
        $highestRisk = 'automatic';
        $riskWeight = ['automatic' => 0, 'confirm' => 1, 'strong-confirm' => 2, 'blocked' => 3];
        $virtual = $this->virtualContext($context);

        foreach ($actions as $index => $action) {
            $definition = $this->registry->find($action['key']);
            if (! $definition) {
    ...
    public function audit(User $actor, array $data, ?string $ip, ?string $userAgent): EcosystemAuditLog
    {
