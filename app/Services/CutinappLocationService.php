...
class CutinappLocationService
{
    public function canonicalMunicipality(?int $cityId, ?string $uf = null): ?BrazilianMunicipality
    {
        ...
        if (! $city) {
            throw ValidationException::withMessages([
                'city_id' => ['Selecione uma cidade válida da lista.'],
            ]);
        }

        if ($uf && strtoupper(trim($uf)) !== $city->uf) {
            throw ValidationException::withMessages([
                'city_id' => ['A cidade selecionada não pertence à UF informada.'],
            ]);
        }
    ...
    public function applyCanonicalCity(array &$data, bool $physicalLocation = true): void
    {
        ...
        if (! $cityId && $legacyCity !== '') {
            $query = BrazilianMunicipality::query()
                ->where('normalized_name', $this->normalizedName($legacyCity));

            if ($uf !== '') {
                $query->where('uf', $uf);
            }

            // Legacy clients historically sent only the city text. Preserve that
            // contract when the text identifies exactly one municipality. If the
            // name is ambiguous across states, require the canonical city/UF
            // selection instead of guessing and persisting the wrong location.
    ...
    public function normalizeCep(?string $cep): ?string
    {
        ...
        if (strlen($digits) !== 8) {
            throw ValidationException::withMessages([
                'cep' => ['Informe um CEP brasileiro válido com 8 números.'],
            ]);
        }
