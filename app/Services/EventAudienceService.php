...
class EventAudienceService
{
    ...
    {
        $order=CommerceOrder::query()->with(['event.application','production.user','user','items'])->find($orderId);
        ...
        $buyerAlreadyNotified=AppNotification::query()
            ->where('app_id',$appId)
            ->where('user_id',$order->user_id)
            ->where('type','ticket_purchase_confirmed')
            ->where('data->order_id',$order->id)
            ->exists();

        if(!$buyerAlreadyNotified){
