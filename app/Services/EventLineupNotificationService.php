...
class EventLineupNotificationService
{
        ...
        foreach($event->artists->where('app_id',(int)$event->app_id)->where('is_published',true) as $artist){
            ...
            foreach($followers as $userId){
                $exists=AppNotification::query()->where('app_id',$event->app_id)->where('user_id',$userId)->where('type','artist_lineup')
                    ->where('reference_type','event')->where('reference_id',$event->id)->where('data->artist_id',$artist->id)->exists();
                if($exists) continue;
                AppNotification::create(['app_id'=>$event->app_id,'user_id'=>$userId,'type'=>'artist_lineup','title'=>$artist->stage_name.' confirmado em evento',
                    'message'=>$artist->stage_name.' fará parte de '.$event->title.'.','reference_type'=>'event','reference_id'=>$event->id,
                    'reference_url'=>'/event/'.$event->slug,'data'=>['artist_id'=>$artist->id,'event_id'=>$event->id]]);
            }
        }
    }
}
