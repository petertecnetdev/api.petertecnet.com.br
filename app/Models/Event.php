...
class Event extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (Event $event) {
            ...
            foreach(['event_format','city_id','cep','address_number','neighborhood','address_complement','address_reference','formatted_address','place_id','latitude','longitude','google_maps_url','online_platform','online_url','online_instructions'] as $field){if(request()->exists($field))$event->setAttribute($field,request()->input($field));}
            ...
            $physical=in_array($event->event_format,['in_person','hybrid'],true);$locationDirty=!$event->exists||$event->isDirty(['city_id','city','uf','cep','event_format']);
            if($locationDirty){$service=app(CutinappLocationService::class);if($event->cep)$event->cep=$service->normalizeCep($event->cep);if($physical){$data=['city_id'=>$event->city_id,'city'=>$event->city,'uf'=>$event->uf];$service->applyCanonicalCity($data,true);$event->city_id=$data['city_id'];$event->city=$data['city'];$event->uf=$data['uf'];$event->state=$data['uf'];if(!trim((string)$event->address))throw ValidationException::withMessages(['address'=>['Informe o endereço do evento presencial.']]);}}
            ...
            if(!$event->start_date||!$event->end_date)return;$start=Carbon::parse($event->start_date,config('app.timezone'));$end=Carbon::parse($event->end_date,config('app.timezone'));if($event->isDirty('start_date')&&$start->lt(now()->subMinutes(1)))throw ValidationException::withMessages(['start_date'=>['O início do evento não pode ficar no passado.']]);if(!$end->gt($start))throw ValidationException::withMessages(['end_date'=>['O término do evento precisa ser posterior ao início.']]);
        });
    }
