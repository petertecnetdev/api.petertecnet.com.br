...
class HomeController extends Controller
{
    ...
    ->whereHas('establishment', fn($q) => $q->whereIn('id', $establishmentIds))
...
private function getItems($establishmentIds)
{
    return Item::where('entity_name','establishment')
        ...
        ->with([
            'establishment:id,name,slug,city,uf,app_id',
    ...
    Establishment::where('app_id',$appId)
        ->withCount([
            'views as total_views' =>
                fn($q) => $q->where('interaction_type','view')
        ])
    ...
    OrderItem::whereHas('order', fn($o) =>
            $o->whereIn('establishment_id', $establishmentIds)
              ->where('created_at','>=',now()->subDays(30))
        )
        ->select('item_id', DB::raw('COUNT(*) as total'))
        ->groupBy('item_id')
    ...
    private function scopeApp( $query, $app_id ) {
    return $query->whereHas('establishment', fn($q) => 
