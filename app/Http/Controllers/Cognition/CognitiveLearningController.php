...
class CognitiveLearningController extends CognitionController
{
    ...
    public function memories(Request $request, CognitiveAgent $agent) { $this->authorizeCognition($request); $q=$agent->memories()->latest(); if($request->filled('type'))$q->where('memory_type',(string)$request->string('type')); if(!$request->boolean('include_inactive'))$q->where('active',true); return response()->json($q->paginate(min(100,max(1,(int)$request->integer('per_page',30))))); }
    public function beliefs(Request $request, CognitiveAgent $agent) { $this->authorizeCognition($request); $q=$agent->beliefs()->orderByDesc('confidence')->latest('last_evidence_at'); if($request->filled('status'))$q->where('status',(string)$request->string('status')); return response()->json($q->paginate(min(100,max(1,(int)$request->integer('per_page',30))))); }
