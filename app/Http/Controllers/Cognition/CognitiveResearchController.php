...
{
    public function framework(Request $request, CognitiveLearningService $learning)
    {
        $this->authorizeCognition($request);
        return response()->json($learning->researchFramework());
    }

    public function bootstrap(Request $request, CognitiveLearningService $learning)
    {
        $this->authorizeCognition($request);
    ...
    public function runs(Request $request, CognitiveAgent $agent, CognitiveQueryService $queries)
    {
        $this->authorizeCognition($request);
        return response()->json($queries->paginateExperimentRuns($agent, (int) $request->integer('per_page', 50)));
    }
    ...
    public function recordRun(Request $request, CognitiveAgent $agent, CognitiveExperiment $experiment, CognitiveLearningService $learning)
    {
        ...
        $data = $request->validate([
            ...
            'result' => ['nullable', 'in:pass,fail,inconclusive'],
