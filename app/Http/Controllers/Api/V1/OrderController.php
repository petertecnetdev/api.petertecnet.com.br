...
class OrderController extends Controller
{
    ...
    {
        $this->context->requireCapability('metrics');
    ...
    private function createPixPayment(Order $order, Establishment $establishment, $user): array
    {
        ...
        $reference = 'order-' . $this->context->id() . '-' . $order->id . '-' . $publicId;
        ...
        try {
            ...
            ], 'order-' . $this->context->id() . '-' . $order->id);
            ...
            return [
                ...
                'amount' => (float) $order->total_price,
                'retryable' => true,
                'message' => 'O provedor de Pix não respondeu, mas o pedido foi criado e preservado.',
            ];
        }
    }
    ...
    private function manageable(Request $request, int $id): Establishment
    {
        ...
        $isMember = Employer::query()
            ->where('establishment_id', $establishment->id)
            ->where('user_id', $userId)
            ->exists();

        abort_unless($isOwner || $isMember, 403, 'Você não possui acesso a esta operação.');
    ...
    private function orderingConfig(Establishment $establishment): array
    {
        ...
        $pixConfigured = $this->context->supports('payments') && (
            trim((string) config('services.mercadopago.access_token')) !== '' || ! empty($establishment->pix_key)
        );
