...
class PlatOrderController extends Controller
{
    ...
    {
        $establishment = $this->publicRestaurant($slug);
        $items = Item::query()
    ...
    public function checkout(Request $request): JsonResponse
    {
        ...
        $payment = $data['payment_method'] === 'pix'
            ? $this->createPixPayment($order, $establishment, $user)
            : null;
    ...
    private function createPixPayment(Order $order, Establishment $establishment, $user): array
    {
        ...
        if ($token === '') {
            ...
            return [
                ...
                'status' => 'failed',
                'amount' => (float) $order->total_price,
                'message' => 'Pix temporariamente indisponível. O pedido foi preservado; escolha outra forma de pagamento com o restaurante.',
            ];
        }
    ...
    private function publicRestaurant(string $slug): Establishment
    {
    ...
    private function restoreStock(Order $order): void
    {
        foreach ($order->items as $line) {
            if ($line->item_id) {
                Item::whereKey($line->item_id)->increment('stock', (int) $line->quantity);
            }
            ...
            foreach ($additionIds as $modifierId) {
                if ($modifierId) {
                    Item::whereKey($modifierId)->increment('stock', (int) $line->quantity);
                }
            }
        }
    }
    ...
    private function orderingConfig(Establishment $establishment): array
    {
        ...
        return [
            ...
            'unavailable_reason' => ! $orderingEnabled
                ...
                : (! $acceptingOrders ? 'O restaurante pausou novos pedidos.' : (! $openNow ? 'O restaurante está fechado agora.' : null)),
            'fulfillment' => [
    ...
    private function isOpenNow(Establishment $establishment): bool
    {
        ...
        foreach ([0, -1] as $offset) {
            ...
            if ($ranges === true || $ranges === '24h') {
                return true;
            }
            if (! is_array($ranges)) {
                continue;
            }
            if (isset($ranges['open'], $ranges['close'])) {
                $ranges = [$ranges];
            }

            foreach ($ranges as $range) {
                if (! is_array($range) || empty($range['open']) || empty($range['close'])) {
                    continue;
                }
