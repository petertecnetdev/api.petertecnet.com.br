...
class CutinappLocationController extends Controller
{
    ...
    public function cities(Request $request, CutinappLocationService $locations)
    {
        $data = $request->validate(['uf' => 'required|string|size:2', 'q' => 'nullable|string|max:120']);
        $uf = strtoupper($data['uf']);
        if (! in_array($uf, self::UFS, true)) throw ValidationException::withMessages(['uf' => ['Selecione uma UF válida.']]);

        $this->ensureStateCached($uf, $locations);
        $query = BrazilianMunicipality::query()->where('uf', $uf);
        if ($term = trim((string) ($data['q'] ?? ''))) {
            $normalized = $locations->normalizedName($term);
            $query->where('normalized_name', 'like', $normalized . '%');
        }
        return response()->json(['cities' => $query->orderBy('name')->limit(30)->get(['ibge_code','name','uf','slug'])]);
    }

    public function cep(string $cep, CutinappLocationService $locations)
    {
        $digits = $locations->normalizeCep($cep);
        $payload = Cache::remember("cutinapp:cep:{$digits}", now()->addDays(30), function () use ($digits) {
        ...
        if (! $payload) return response()->json(['message' => 'Não foi possível localizar este CEP agora. Você pode preencher o endereço manualmente.'], 404);
    ...
    private function ensureStateCached(string $uf, CutinappLocationService $locations): void
    {
        if (BrazilianMunicipality::query()->where('uf', $uf)->exists()) return;
        Cache::lock("cutinapp:municipalities:{$uf}", 15)->block(5, function () use ($uf, $locations) {
            if (BrazilianMunicipality::query()->where('uf', $uf)->exists()) return;
            $response = Http::timeout(8)->retry(2, 250)->acceptJson()->get("https://servicodados.ibge.gov.br/api/v1/localidades/estados/{$uf}/municipios");
            if (! $response->successful()) throw ValidationException::withMessages(['city_id' => ['A lista oficial de cidades está temporariamente indisponível. Tente novamente em instantes.']]);
            $now = now();
