...
class EventController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($request->hasFile('image')) {
                Log::info('Imagem do evento fornecida, processando...');

                // Definir o caminho do diretório público para imagens
                $destinationPath = public_path('images');

                // Gerar um nome único para a imagem
                $imageName = uniqid('event_') . '.' . $request->file('image')->getClientOriginalExtension();

                // Mover a imagem para o diretório público "images"
                $request->file('image')->move($destinationPath, $imageName);

                // Redimensionar a imagem para 850x450
    ...
    public function view($slug)
    {
        ...
        $userHdesconfirmedPost = false; // Default value
        ...
        if (Auth::check()) {
            ...
            $userHasLikedPost = $interactionLike ? true : false;
            $userHdesconfirmedPost = $interactionConfirm ? true : false;
        }

        return response()->json([
            'event' => $event,
            'events' => $events,
            'tickets' => $event->tickets, // Incluindo os ingressos na resposta
