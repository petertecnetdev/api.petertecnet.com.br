...
class SupportController extends Controller
{
    ...
    public function index(Request $request)
    {
        $query = SupportTicket::query()
            ->with([
                ...
                'requester:id,first_name,last_name,user_name,email',
                'assignee:id,first_name,last_name,user_name,email',
            ]);

        if ($request->filled('status')) {
            $statuses = array_values(array_intersect(explode(',', (string) $request->query('status')), self::STATUSES));
            if ($statuses) {
                $query->whereIn('status', $statuses);
            }
        }
        if ($request->filled('priority')) {
            $priorities = array_values(array_intersect(explode(',', (string) $request->query('priority')), self::PRIORITIES));
            if ($priorities) {
        ...
        if ($request->filled('assigned_to_user_id')) {
            ...
            $value === 'unassigned'
                ...
                : $query->where('assigned_to_user_id', (int) $value);
        }
        if ($request->filled('q')) {
    ...
    public function show(SupportTicket $ticket)
    {
        ...
        return response()->json(['ticket' => $this->ticketPayload($ticket, true)]);
    }
    ...
    public function update(Request $request, SupportTicket $ticket)
    {
        ...
        DB::transaction(function () use ($request, $ticket, $updates, $before) {
            ...
            if ($changes) {
                SupportMessage::create([
                    ...
                    'author_type' => 'system',
                    'body' => 'Chamado atualizado pela equipe de suporte.',
                    'is_internal' => true,
    ...
    public function reply(Request $request, SupportTicket $ticket)
    {
        ...
        DB::transaction(function () use ($ticket, $admin, $data, $internal) {
            SupportMessage::create([
                'support_ticket_id' => $ticket->id,
                'user_id' => $admin?->id,
                'author_type' => 'agent',
    ...
    private function ticketPayload(SupportTicket $ticket, bool $withMessages): array
    {
        $payload = [
            ...
            'requester' => $ticket->requester ? [
                ...
                'name' => $this->userName($ticket->requester),
                'email' => $ticket->requester->email,
            ] : null,
            'assignee' => $ticket->assignee ? [
                'id' => $ticket->assignee->id,
                'name' => $this->userName($ticket->assignee),
        ...
        if ($withMessages) {
            $payload['messages'] = $ticket->messages->map(fn (SupportMessage $message) => [
                'id' => $message->id,
                'author_type' => $message->author_type,
                'author_name' => $message->author_type === 'agent'
                    ? ($message->user ? $this->userName($message->user) : 'Equipe Peter Tecnet')
                    : ($message->author_type === 'system' ? 'Sistema' : $ticket->requester_name),
                'body' => $message->body,
