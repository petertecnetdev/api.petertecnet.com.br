...
class ImportantEventController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $query = $this->filteredQuery($request)
            ...
            ->with(['reads' => fn ($reads) => $reads->where('user_id', $userId)->select('id', 'important_event_id', 'user_id', 'read_at')]);
        ...
        $summaryTotal = (clone $base)->count();
        $summaryUnread = (clone $base)
    ...
    {
        ImportantEventRead::query()->updateOrCreate(
            [
    ...
    public function markAllRead(Request $request): JsonResponse
    {
        ...
        ImportantEvent::query()->select('id')->orderBy('id')->chunkById(500, function ($events) use ($userId, $now, &$updated) {
            ...
            DB::table('important_event_reads')->upsert(
                $rows,
                ['important_event_id', 'user_id'],
                ['read_at', 'updated_at'],
            );
            $updated += count($rows);
        });
    ...
    private function filteredQuery(Request $request)
    {
        ...
        if ($request->filled('app_id')) {
            $query->where('app_id', (int) $request->integer('app_id'));
        }
