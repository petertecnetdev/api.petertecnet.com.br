...
class AdminUserDetailController extends Controller
{
    ...
    public function activity(Request $request, User $user): JsonResponse
    {
        ...
        if (! empty($data['type'])) {
            $query->where('interaction_type', $data['type']);
        }
        if (! empty($data['outcome'])) {
            $query->where('outcome', $data['outcome']);
        }
        if (! empty($data['severity'])) {
            $query->where('severity', $data['severity']);
        }
        if (! empty($data['environment'])) {
            $query->where('environment', $data['environment']);
        }
    ...
    private function resources(User $user, Collection $applications): array
    {
        ...
        $teamMembers = $establishmentIds->isEmpty()
            ? collect()
            : Employer::query()
        ...
        $ordersQuery = Order::query()->where(function (Builder $query) use ($user, $employmentIds, $establishmentIds) {
            ...
            if ($employmentIds->isNotEmpty()) {
                $query->orWhereIn('attendant_id', $employmentIds);
            }
            if ($establishmentIds->isNotEmpty()) {
                $query->orWhere(function (Builder $entityQuery) use ($establishmentIds) {
                    $entityQuery->whereIn('entity_id', $establishmentIds)
        ...
        return [
            ...
            'employments' => $this->resourceGroup('Vínculos como employer', $employments->count(), $employments->take(self::RESOURCE_LIMIT)->map(fn (Employer $row) => $this->employmentPayload($row, $applications))),
            'team_members' => $this->resourceGroup('Equipe dos estabelecimentos', $teamMembers->count(), $teamMembers->map(fn (Employer $row) => $this->teamPayload($row, $applications))),
    ...
    private function resourceGroup(string $label, int $total, Collection $rows): array
    {
        return [
            'label' => $label,
            'total' => $total,
            'truncated' => $total > self::RESOURCE_LIMIT,
            'data' => $rows->values(),
        ];
    }
    ...
    private function interactionQuery(User $user): Builder
    {
        return Interaction::query()
            ->where('user_id', $user->id)
            ->with('application:id,name,slug,logo,url');
    }
    ...
    private function establishmentPayload($row, Collection $applications): array
    {
        return [
            'id' => $row->id,
            'app_id' => $row->app_id,
            'application' => $this->appPayload($applications, $row->app_id),
    ...
    private function itemPayload(Item $row, Collection $applications): array
    {
        ...
        return [
            'id' => $row->id,
            'app_id' => $appId,
            'application' => $this->appPayload($applications, $appId),
            'name' => $row->name,
            'slug' => $row->slug,
            'type' => $row->type,
            'category' => $row->category,
    ...
    private function employmentPayload(Employer $row, Collection $applications): array
    {
        ...
        return [
            ...
            'establishment' => $row->establishment ? [
                ...
                'category' => $row->establishment->category,
            ] : null,
            'created_at' => $row->created_at,
            'updated_at' => $row->updated_at,
        ];
    ...
    private function eventPayload(Event $row, Collection $applications): array
    {
        return [
            ...
            'production' => $row->production ? [
                ...
                'name' => $row->production->fantasy ?: $row->production->name,
    ...
    private function orderPayload(Order $row, Collection $applications, User $user, Collection $employmentIds, Collection $establishmentIds): array
    {
        ...
        return [
            ...
            'app_id' => $row->app_id,
            'application' => $this->appPayload($applications, $row->app_id),
            'name' => $row->order_number ?: "Pedido #{$row->id}",
            ...
            'link_types' => $links->unique()->values(),
            'created_at' => $row->created_at,
            'updated_at' => $row->updated_at,
        ];
    }
    ...
    private function passPayload(EventPass $row, Collection $applications): array
    {
        ...
        return [
            ...
            'app_id' => $appId,
            'application' => $this->appPayload($applications, $appId),
            'name' => $row->event?->title ?: "Ingresso #{$row->id}",
            'status' => $row->status,
    ...
    private function ticketPayload(Ticket $row, Collection $applications): array
    {
        return [
            'id' => $row->id,
            'app_id' => $row->app_id ?: $row->event?->app_id,
            'application' => $this->appPayload($applications, $row->app_id ?: $row->event?->app_id),
            'name' => $row->name,
            'type' => $row->type,
    ...
    private function appPayload(Collection $applications, $appId): ?array
    {
        ...
        return $applications->get((int) $appId)?->toArray();
    }
