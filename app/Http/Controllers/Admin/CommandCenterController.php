...
{
    /**
     * Mission Control must never depend on one optional telemetry table to render.
     * Every snapshot below degrades gracefully when a capability is unavailable.
     */
    public function overview(Request $request): JsonResponse
    {
        ...
        $apps = Schema::hasTable('applications')
            ? Application::query()->orderBy('name')->get($columns ?: ['id'])
            : collect();
        ...
        $applications = $apps->map(function ($app) use ($now, $hasAppId, $hasOutcome, $hasDuration) {
            $base = Schema::hasTable('interactions') && $hasAppId
                ? DB::table('interactions')->where('app_id', $app->id)
                : null;

            $lastActivity = $base ? (clone $base)->max('created_at') : null;
            $requests24h = $base ? (clone $base)->where('created_at', '>=', $now->copy()->subDay())->count() : 0;
            $errors24h = $base && $hasOutcome
                ? (clone $base)->where('created_at', '>=', $now->copy()->subDay())->where('outcome', 'error')->count()
                : 0;
            $latency = $base && $hasDuration
                ? (clone $base)->where('created_at', '>=', $now->copy()->subHour())->whereNotNull('duration_ms')->avg('duration_ms')
                : null;

            $probe = Schema::hasTable('admin_service_probes')
                ? DB::table('admin_service_probes')->where('application_id', $app->id)->orderByDesc('checked_at')->first()
                : null;

            $active = property_exists($app, 'is_active') || isset($app->is_active) ? (bool) $app->is_active : true;
            $status = ! $active
                ? 'disabled'
                : ($probe?->status ?: ($lastActivity && strtotime((string) $lastActivity) >= $now->copy()->subDay()->timestamp ? 'operational' : 'unknown'));
        ...
        if (($runtime['scheduler']['status'] ?? 'unknown') === 'failed') {
            $score -= 15;
        }
        $score = max(0, $score);
    ...
    public function queues(Request $request): JsonResponse
    {
        ...
        $snapshot['failed_rows'] = Schema::hasTable('failed_jobs')
    ...
    public function incidents(Request $request): JsonResponse
    {
        ...
        if ($request->filled('status')) {
            $query->where('i.status', $request->string('status')->toString());
        }
        if ($request->filled('severity')) {
            $query->where('i.severity', $request->string('severity')->toString());
        }
    ...
    public function updateIncident(Request $request, int $incident): JsonResponse
    {
        ...
        if (($patch['status'] ?? null) === 'acknowledged') {
    ...
    public function globalSearch(Request $request): JsonResponse
    {
        ...
        if (Schema::hasTable('users')) {
            $groups['users'] = DB::table('users')
                ->select(array_values(array_intersect(['id', 'first_name', 'last_name', 'user_name', 'email', 'created_at'], Schema::getColumnListing('users'))))
                ->where(function ($query) use ($like, $q) {
                    foreach (['email', 'user_name', 'first_name', 'last_name'] as $column) {
        ...
        if (Schema::hasTable('applications')) {
            $groups['applications'] = DB::table('applications')
                ->select(array_values(array_intersect(['id', 'name', 'slug', 'url', 'version', 'is_active'], Schema::getColumnListing('applications'))))
        ...
        if (Schema::hasTable('establishments')) {
            $columns = array_values(array_intersect(['id', 'name', 'fantasy', 'slug', 'city', 'uf', 'app_id'], Schema::getColumnListing('establishments')));
            $groups['establishments'] = DB::table('establishments')->select($columns)
                ->where(function ($query) use ($like) {
                    foreach (['name', 'fantasy', 'slug', 'cnpj'] as $column) {
        ...
        if (Schema::hasTable('items')) {
            $columns = array_values(array_intersect(['id', 'name', 'slug', 'price', 'app_id', 'entity_name', 'entity_id', 'category'], Schema::getColumnListing('items')));
            $groups['items'] = DB::table('items')->select($columns)
                ->where(function ($query) use ($like) {
                    foreach (['name', 'slug', 'sku', 'category'] as $column) {
        ...
        return response()->json([
            'query' => $q,
            'groups' => $groups,
            'total' => collect($groups)->sum(static fn ($rows) => count($rows)),
        ]);
    }

    /**
     * The operational intelligence UI is optional. Keeping these contracts stable
     * prevents one evolving subsystem from taking down the entire Mission Control.
     */
    ...
    public function intelligence(Request $request): JsonResponse
    {
        $this->authorizeAccess($request);
        $security = $this->securitySnapshot();
        $activeAlerts = (int) ($security['critical_events_24h'] ?? 0);
    ...
    {
        $this->authorizeAccess($request);
        abort(404, 'Problema operacional não encontrado.');
    }
    ...
    {
        $scheduler = Schema::hasTable('admin_runtime_heartbeats')
            ? DB::table('admin_runtime_heartbeats')->where('service', 'scheduler')->first()
            : null;
        $backup = Schema::hasTable('admin_runtime_heartbeats')
            ? DB::table('admin_runtime_heartbeats')->where('service', 'backup')->first()
            : null;
        ...
        return [
            ...
            'backup' => [
                ...
                'last_success_at' => $backup?->last_seen_at,
                'meta' => $backup?->meta ? json_decode($backup->meta, true) : null,
            ],
    ...
    private function securitySnapshot(bool $detail = false): array
    {
        ...
        $payload = [
            'critical_events_24h' => $critical + $suspicious,
        ...
        if ($detail) {
            ...
            if (in_array('severity', $columns, true)) {
                $query->whereIn('severity', ['attention', 'suspicious', 'critical']);
            } elseif (in_array('outcome', $columns, true)) {
                $query->whereIn('outcome', ['denied', 'error']);
            }
    ...
    private function authorizeAccess(Request $request): void
    {
        ...
        ), 403, 'Usuário sem permissão para acessar o Mission Control.');
    }
