...
class AdminEstablishmentResourceController extends Controller
{
    public function context(Request $request, Establishment $establishment): JsonResponse
    {
        ...
        $items = Item::query()
            ...
            ->where('app_id', $appId)
            ->where('status', true)
            ->orderByRaw("CASE WHEN type = 'service' THEN 0 ELSE 1 END")
            ->orderBy('name')
    ...
    public function storeEmployer(Request $request, Establishment $establishment): JsonResponse
    {
        ...
        if ((int) $data['user_id'] === (int) $establishment->user_id) {
            throw ValidationException::withMessages([
                'user_id' => ['O proprietário não deve ser cadastrado como employer do próprio establishment.'],
            ]);
        }

        if (Employer::query()->where('user_id', $data['user_id'])->where('establishment_id', $establishment->id)->exists()) {
            throw ValidationException::withMessages([
                'user_id' => ['Este usuário já está vinculado ao establishment como employer.'],
            ]);
        }
        ...
        try {
            if ($employer->establishment?->user?->email) {
                Mail::to($employer->establishment->user->email)->queue(new OwnerNotifiedNewCollaborator($employer->establishment, $employer));
            }
            if ($employer->user?->email) {
                Mail::to($employer->user->email)->queue(new NewEmployerCollaborator($employer->establishment, $employer));
            }
        } catch (\Throwable $exception) {
            Log::warning('AdminEstablishmentResourceController.storeEmployer.mail', [
    ...
    public function storeAppointment(Request $request, Establishment $establishment): JsonResponse
    {
        ...
        $employer = Employer::query()
            ->where('establishment_id', $establishment->id)
            ->findOrFail((int) $data['attendant_id']);
        ...
        if ($validItems !== $itemIds->count()) {
            throw ValidationException::withMessages([
                'items' => ['Um ou mais itens não pertencem a este establishment/aplicação ou estão inativos.'],
            ]);
        }
        ...
        if ((int) $client->id === (int) $employer->user_id) {
            throw ValidationException::withMessages([
                'client_id' => ['O cliente não pode ser o mesmo usuário do employer selecionado.'],
            ]);
        }
    ...
    private function assertApplicationLinked(Establishment $establishment, int $appId): void
    {
        ...
        if (! $linked) {
            throw ValidationException::withMessages([
                'app_id' => ['A aplicação informada não está vinculada a este establishment.'],
            ]);
        }
    }
