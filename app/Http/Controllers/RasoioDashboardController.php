...
class RasoioDashboardController extends ApiController
{
    ...
    public function overview(Request $request, string $slug)
    {
        ...
        abort_unless(
            ...
            403,
            'Somente a gestão da barbearia pode visualizar estes indicadores.'
        );
        ...
        foreach ($orders as $order) {
            ...
            if ($status === 'pending') {
                ...
                $completionAt = $order->attended_at
                    ...
                    : $scheduledEnd;
                $completionHour = (int) $completionAt->format('G');
                $timeline[$completionHour]['completed']++;
