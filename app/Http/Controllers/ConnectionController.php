...
use App\Events\ApplicationUserEvent;
use App\Models\User;
...
class ConnectionController extends Controller
{
    ...
    public function uploadPhoto(Request $request)
    {
        ...
        if (! $profile) return response()->json(['message' => 'Complete seu perfil antes de adicionar fotos.'], 422);
        $count = DB::table('laora_photos')->where('profile_id', $profile->id)->count();
        if ($count >= self::PHOTO_LIMIT) return response()->json(['message' => 'Você pode manter no máximo 6 fotos no perfil.'], 422);
        $file = $request->file('photo');
        $filename = Str::uuid() . '.' . strtolower($file->getClientOriginalExtension());
        $path = $file->storeAs('profiles/' . $this->context->slug() . '/' . $request->user()->id, $filename, 'public');
        $id = DB::table('laora_photos')->insertGetId([
            'profile_id' => $profile->id, 'path' => $path, 'position' => $count,
            'is_primary' => false, 'moderation_status' => 'pending', 'created_at' => now(), 'updated_at' => now(),
        ]);
        $this->refreshProfileCompleteness((int) $profile->id);
        return response()->json(['data' => $this->photoPayload(DB::table('laora_photos')->where('id', $id)->first(), true), 'message' => 'Foto enviada e aguardando moderação.'], 201);
    }
    ...
    public function deletePhoto(Request $request, int $photoId)
    {
        ...
        if (! $photo) return response()->json(['message' => 'Foto não encontrada.'], 404);
    ...
    public function reorderPhotos(Request $request)
    {
        ...
        DB::transaction(function () use ($profile, $data) {
            foreach ($data['photo_ids'] as $position => $photoId) {
                DB::table('laora_photos')->where('profile_id', $profile->id)->where('id', $photoId)->update(['position' => $position, 'is_primary' => false, 'updated_at' => now()]);
            }
            $this->normalizePhotos((int) $profile->id);
        });
        return $this->profile($request);
    ...
    {
        $this->context->requireCapability('matching');
        $user = $request->user();
        $this->assertActive($user);
        $this->assertVerified($user);
        $me = DB::table('laora_profiles')->where('user_id', $user->id)->first();
        if (! $me || ! $me->is_complete || ! $me->discovery_enabled) return response()->json(['data' => [], 'meta' => ['profile_required' => true]]);
        DB::table('laora_profiles')->where('id', $me->id)->update(['last_active_at' => now()]);
        $blockedIds = DB::table('laora_blocks')->where('blocker_user_id', $user->id)->orWhere('blocked_user_id', $user->id)->get()->map(fn ($row) => (int) ($row->blocker_user_id == $user->id ? $row->blocked_user_id : $row->blocker_user_id))->all();
        ...
        if ($preferred) $query->whereIn('gender', $preferred);
    ...
    public function swipe(Request $request)
    {
        ...
        DB::transaction(function () use ($user, $targetId, $data, &$match) {
            DB::table('laora_swipes')->updateOrInsert(['swiper_user_id' => $user->id, 'target_user_id' => $targetId], ['action' => $data['action'], 'updated_at' => now(), 'created_at' => now()]);
            ...
            DB::table('laora_matches')->updateOrInsert(['user_one_id' => $one, 'user_two_id' => $two], ['status' => 'active', 'matched_at' => now(), 'unmatched_at' => null, 'unmatched_by_user_id' => null, 'updated_at' => now(), 'created_at' => now()]);
    ...
    public function matches(Request $request)
    {
        ...
        $query = DB::table('laora_matches')->where('status', 'active')->where(fn ($q) => $q->where('user_one_id', $user->id)->orWhere('user_two_id', $user->id));
        ...
        return response()->json(['data' => $rows->map(fn ($match) => $this->matchPayload($match, $user->id))->values(), 'meta' => ['has_more' => $hasMore, 'next_before_id' => $rows->last()?->id]]);
    }
    ...
    public function unmatch(Request $request, int $matchId)
    {
        ...
        DB::table('laora_matches')->where('id', $match->id)->update(['status' => 'unmatched', 'unmatched_at' => now(), 'unmatched_by_user_id' => $request->user()->id, 'updated_at' => now()]);
    ...
    {
        $this->context->requireCapability('messaging');
        $user = $request->user();
        $this->assertActive($user);
        $this->assertVerified($user);
        ...
        return response()->json(['data' => $messages, 'meta' => ['has_more' => $hasMore, 'next_before_id' => $messages->first()?->id]]);
    }
    ...
    public function sendMessage(Request $request, int $matchId)
    {
        ...
        $id = DB::table('laora_messages')->insertGetId(['match_id' => $match->id, 'sender_user_id' => $user->id, 'body' => $body, 'created_at' => now(), 'updated_at' => now()]);
    ...
    public function block(Request $request, int $targetUserId)
    {
        ...
        DB::transaction(function () use ($userId, $targetUserId, $data) {
            DB::table('laora_blocks')->updateOrInsert(['blocker_user_id' => $userId, 'blocked_user_id' => $targetUserId], ['reason' => $data['reason'] ?? null, 'updated_at' => now(), 'created_at' => now()]);
            [$one, $two] = $this->orderedPair($userId, $targetUserId);
            DB::table('laora_matches')->where('user_one_id', $one)->where('user_two_id', $two)->update(['status' => 'blocked', 'unmatched_at' => now(), 'unmatched_by_user_id' => $userId, 'updated_at' => now()]);
        });
    ...
    {
        $this->context->requireCapability('moderation');
        $userId = $request->user()->id;
        $data = $request->validate([
        ...
        $id = DB::table('laora_reports')->insertGetId(['reporter_user_id' => $userId, 'reported_user_id' => $data['reported_user_id'], 'match_id' => $data['match_id'] ?? null, 'reason' => $data['reason'], 'details' => isset($data['details']) ? trim($data['details']) : null, 'status' => 'open', 'created_at' => now(), 'updated_at' => now()]);
        if ($data['block_user'] ?? true) {
            DB::table('laora_blocks')->updateOrInsert(['blocker_user_id' => $userId, 'blocked_user_id' => $data['reported_user_id']], ['reason' => 'report', 'updated_at' => now(), 'created_at' => now()]);
    ...
    private function profilePayload(object $profile, bool $owner): array
    {
        ...
        $payload = [
            ...
            'age' => Carbon::parse($profile->birthdate)->age, 'gender' => $profile->gender, 'orientation' => $profile->orientation,
            'bio' => $profile->bio, 'interests' => $this->decodeJson($profile->interests), 'city' => $profile->city,
            'uf' => $profile->uf, 'photos' => $photos, 'last_active_at' => $profile->last_active_at,
        ];
        if ($owner) {
            $payload += [
                'birthdate' => $profile->birthdate, 'age_min' => $profile->age_min, 'age_max' => $profile->age_max,
                'max_distance_km' => $profile->max_distance_km, 'preferred_genders' => $this->decodeJson($profile->preferred_genders),
                'discovery_enabled' => (bool) $profile->discovery_enabled, 'is_complete' => (bool) $profile->is_complete,
    ...
    private function matchPayload(object $match, int $viewerId): array
    {
        ...
        return ['id' => $match->id, 'status' => $match->status, 'matched_at' => $match->matched_at, 'profile' => $profile ? $this->profilePayload($profile, false) : null, 'last_message' => $lastMessage, 'unread_count' => $unread];
    }

    private function ownedMatch(int $matchId, int $userId, bool $requireActive = false): object
    {
        $match = DB::table('laora_matches')->where('id', $matchId)->where(fn ($q) => $q->where('user_one_id', $userId)->orWhere('user_two_id', $userId))->first();
    ...
    {
        $blocked = DB::table('laora_blocks')->where(fn ($q) => $q->where('blocker_user_id', $one)->where('blocked_user_id', $two))->orWhere(fn ($q) => $q->where('blocker_user_id', $two)->where('blocked_user_id', $one))->exists();
        abort_if($blocked, 403, 'Interação indisponível entre estas contas.');
    }

    private function assertVerified(User $user): void
    {
        abort_unless($user->email_verified_at, 403, 'Verifique seu e-mail para usar descoberta, conexões e mensagens.');
    }

    private function assertActive(User $user): void
    {
        $action = DB::table('laora_moderation_actions')->where('target_user_id', $user->id)->whereIn('action', ['suspend', 'ban'])->where(fn ($q) => $q->whereNull('expires_at')->orWhere('expires_at', '>', now()))->latest('id')->first();
        if ($action) abort(403, $action->action === 'ban' ? 'Sua conta está impedida de usar esta funcionalidade.' : 'Seu acesso está temporariamente suspenso.');
    }
    ...
    private function normalizePhotos(int $profileId): void
    {
        ...
        foreach ($photos as $position => $item) {
            ...
            DB::table('laora_photos')->where('id', $item->id)->update(['position' => $position, 'is_primary' => $isPrimary, 'updated_at' => now()]);
        }
    }
    ...
    {
        try {
            event(new ApplicationUserEvent($this->context->slug(), $userId, $type, $payload));
        } catch (\Throwable $e) {
            report($e);
        }
    }
