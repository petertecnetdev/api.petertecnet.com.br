...
class LocationController extends Controller
{
    ...
    public function cities(Request $request, LocationService $locations)
    {
    ...
    public function cep(string $cep, LocationService $locations)
    {
        $digits = $locations->normalizeCep($cep);
        $payload = Cache::remember("location:cep:{$digits}", now()->addDays(30), function () use ($digits) {
        ...
        if (! $payload) {
            return response()->json(['message' => 'Não foi possível localizar este CEP agora. Você pode preencher o endereço manualmente.'], 404);
        }
    ...
    private function ensureStateCached(string $uf, LocationService $locations): void
    {
        if (BrazilianMunicipality::query()->where('uf', $uf)->exists()) return;

        Cache::lock("municipalities:{$uf}", 15)->block(5, function () use ($uf, $locations) {
            if (BrazilianMunicipality::query()->where('uf', $uf)->exists()) return;
            ...
            if (! $response->successful()) {
                throw ValidationException::withMessages(['city_id' => ['A lista oficial de cidades está temporariamente indisponível. Tente novamente em instantes.']]);
            }
            $now = now();
