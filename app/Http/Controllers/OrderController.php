    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($isScheduled && !empty($data['attendant_id'])) {
                ...
                $exactConflict = \App\Models\Order::where('attendant_id', $employerId)
                    ->where('type', 'appointment')
                    ->where('order_datetime', $slotStart)
                    ->whereIn('appointment_status', ['pending', 'confirmed'])
                    ->exists();

                if ($hasConflict || $exactConflict) {
            ...
            if ($isScheduled && !empty($data['attendant_id'])) {
                ...
                $exactConflict = $existingAppointments->contains(function ($a) use ($appointmentStart) {
                    return Carbon::parse($a->order_datetime)->equalTo($appointmentStart);
                });
