...
class ReportController extends BaseController
{
    public function order(Request $request)
    {
        ...
        $entityName = $request->input('entity_name', 'establishment');
        ...
        $peakHours = $orders
            ->map(fn($o) => $o->order_datetime->format('H'))
            ->countBy()
            ->toArray();
        ...
        $customers = $orders->groupBy('customer_phone')
            ->map(fn($g) => $g->sum('total_price'));
        $topCustomers = $customers
            ->sortDesc()
            ->take(5)
            ->map(fn($amt, $phone) => ['phone' => $phone, 'total' => $amt])
            ->values()
            ->toArray();
